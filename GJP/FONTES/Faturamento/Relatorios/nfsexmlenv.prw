...
static function prestacao( cMunPrest, cDescMunP, aDest, cMunPSIAFI )
	...
	If ValType(aMvEndPres) <> "U" // Exemplo de preenchimento do parâmetro {SC5->C5_ENDPRES,SC5->C5_NUMPRES,SC5->C5_COMPPRE,SC5->C5_BAIPRES,SC5->C5_CEPPRES}
		cLogradPres	:= AllTrim(If(!Empty(aMvEndPres[01]), aMvEndPres[01],"")) //Logradouro da prestação
		cNumEndPres	:= AllTrim(If(!Empty(aMvEndPres[02]), aMvEndPres[02],"")) //Número do logradouro da prestação
		cCompEndPres	:= AllTrim(If(!Empty(aMvEndPres[03]), aMvEndPres[03],"")) //Complemento do logradouro da prestação
		cBairroPres	:= AllTrim(If(!Empty(aMvEndPres[04]), aMvEndPres[04],"")) //Bairro do logradouro da prestação
		cCepPres	:= AllTrim(If(!Empty(aMvEndPres[05]), aMvEndPres[05],""))//Cep da prestação
	EndIf	
...
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe,aCST, cTpPessoa, cCodMun, cF4Agreg, nDescon   )	
	...
	Local  aRestImp    := {}	
	Local  lIntegHtl	  := SuperGetMv("MV_INTHTL",, .F.) //Integracao via Mensagem Unica - Hotelaria
	Local lFata910 := SuperGetMV( "MV_DESCNFS", .F., .F. ) 	
	Local lDescOBS := SuperGetMV( "MV_OBSNFSE", .F., .F. )
	Local cDescNFSe := ""
			...
			Else
				If cMvMsgTrib $ "1-3" .And. cTpCliente == "F" .And. ( ( aProd[Nx][38] + aProd[Nx][39] + aProd[Nx][40] ) > 0 )
					...
					If aProd[Nx][38] > 0
						...
						cCrgTrib += 'R$ ' + ConvType( aProd[Nx][38], 15, 2 )
						If Val(cPercTrib) > 0
							cCrgTrib += " (" + cPercTrib + "%)"
						Endif
						cCrgTrib += " Federal"
					...
					If aProd[Nx][39] > 0
						...
						If !Empty(cCrgTrib)
							cCrgTrib	+= " e "
						Endif
						cCrgTrib += "R$ " + ConvType( aProd[Nx][39], 15, 2 )
						If Val(cPercTrib) > 0
							cCrgTrib += " (" + cPercTrib + "%)" 
						Endif
						cCrgTrib += " Estadual"
					...
					If aProd[Nx][40] > 0
						...
						If !Empty(cCrgTrib)
							cCrgTrib	+= " e "
						Endif
						cCrgTrib += "R$ " + ConvType( aProd[Nx][40], 15, 2 )
						If Val(cPercTrib) > 0
							cCrgTrib += " (" + cPercTrib + "%)"
						Endif
						cCrgTrib += " Municipal."
...
//			nValLiq	:= aProd[Nx][27] - aPisXml[1] - aCofinsXml[1] - aInssXml[1] - aIRRFXml[1] - aCSLLXml[1] - aIssRet[1]
		...
		//Se estiver utilizando a rotina de descrição personalizada de serviços (FATA910) e não for descrição no campo de informações complementares, então define a descrição do serviço com base no modelo personalizado
		If lFata910 .AND. ! lDescOBS
			cDescNFSe := FT910Desc( cFilAnt )
			cString	+= "<discr>" + AllTrim(cDescNFSe) + cCargaTrb + "</discr>"
		Else
...
static function infCompl( cMensCli, cMensFis, lNFeDesc, cDescrNFSe )	
	...
	//Se estiver utilizando a rotina de descrição personalizada de serviços (FATA910) e for descrição no campo de informações complementares, então define a mensagem com base no modelo personalizado
	If lFata910 .AND. lDescOBS
		cDescNFSe := FT910Desc( cFilAnt )
		cString += "<observacao>" + Alltrim(cDescNFSe) + "</observacao>"
	Else
