...
User Function CPRJW01b(nAprov,_cAssunto, _cNumSSI, _cCpo, oProcess)

Local _dDtAceite:=CtoD('')
_xArea		:= GetArea()
_cSist		:= ""
_cIdent		:= ""
_cDesc		:= ""
...
If _cOpc == "S" // aceite                          
	...
	If DbSeek(xFilial("SZP")+_cNumSSI)
		...
		_dDtAceite	:=SZP->ZP_ACEITE
	Else
		_dDatConclu	:= CTOD(" / / ")
	EndIf
	  
	//Fabio - Se ja houve o aceite, o outro usuแrio que tamb้m deu o aceite nao serแ mais gravado
	If !Empty(_dDtAceite)
		RestArea(_xArea)
		Return
	EndIf
	...
	If DbSeek(xFilial("SZP")+_cNumSSI)
		...
		SZP->ZP_IDAPROV	:=_cCpo//Campo que contem a identificacao de quem aprovou
		MsUnLock()
	EndIf
