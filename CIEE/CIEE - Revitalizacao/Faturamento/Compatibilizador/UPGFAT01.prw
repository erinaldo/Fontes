...
/*/
//--------------------------------------------------------------------
User Function UPGFAT01( cEmpAmb, cFilAmb )
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPBCOM01" )
				Else
					MsgStop( "Atualização não Realizada.", "UPBCOM01" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
// Tabela SC5
//
aAdd( aSX3, {'SC5','A5','C5_XRPSSOC','C',9,0,'Num RPS','Num RPS','Num RPS','Num RPS','Num RPS','Num RPS','','',Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) +Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) +Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(160),'','',0,	Chr(254) + Chr(192),'','','U','S','V','R','','','','','','','','','','',''} )

//
// Tabela SF3
//
aAdd( aSX3, {'SF3','F3','F3_XFLGRET','C',1,0,'FLAG RET','FLAG RET','FLAG RET','FLAG RET','FLAG RET','FLAG RET','','',Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) +Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128) +Chr(128) + Chr(128) + Chr(128) + Chr(128) + Chr(128),'','',0,	Chr(254) + Chr(192),'','','U','S','V','R','','','','','','','','','','',''} )
...
For nI := 1 To Len( aSX3 )
	...
	Else
		...
		If !Empty( SX3->X3_GRPSXG ) .AND. SX3->X3_GRPSXG <> aSX3[nI][nPosSXG]
			...
			If SXG->( MSSeek( SX3->X3_GRPSXG ) )
				If aSX3[nI][nPosTam] <> SXG->XG_SIZE
					...
					AutoGrLog("O tamanho do campo " + aSX3[nI][nPosCpo] + " nao atualizado e foi mantido em ["+;
								AllTrim( Str( SXG->XG_SIZE ) ) + "]"+ CRLF+;
		...
		For nJ := 1 To Len( aSX3[nI] )
			...
			//
			If aEstrut[nJ][1] == SX3->( FieldName( nJ ) ) .AND. ;
				...
				If nOpcA == 1
					AutoGrLog("Alterado o campo " + aSX3[nI][nPosCpo] + CRLF +;
							  "   " + PadR( SX3->( FieldName( nJ ) ), 10 ) + " de [" + AllToChar( SX3->( FieldGet( nJ ) ) ) + "]" + CRLF+ ;
							  "            para [" + AllToChar( aSX3[nI][nJ] )          + "]" + CRLF + CRLF )
...
aAdd( aSIX, {'SC5','6','C5_FILIAL+C5_XRPSSOC','Num RPS','Num RPS','Num RPS','U','','CODRPSSOE','S'} )             
...
// Especifico
//
aAdd( aSX6, {'  ','MV_XCONSOC','C','Cond pagamento para integração com SOC','','','','','','','','','001','','','U',''} )
...
aAdd( aSX6, {'  ','MV_XTESSOC','C','TES a ser utilizada na integração SOC','','','','','','','','','999','','','U',''} )
...
@return Se não for marcada nenhuma o vetor volta vazio
