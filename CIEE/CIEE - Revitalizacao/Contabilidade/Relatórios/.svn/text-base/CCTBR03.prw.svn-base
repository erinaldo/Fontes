...
_cQuery := " SELECT CT2_KEY AS CT2_KEY, CT2_KEY AS FORNECE, "
_cQuery += " CT2_LP, CT2_ITEMD, CT2_ITEMC, CT2_CCC, CT2_CCD, CT2_VALOR, CT2_DC, CT2_DATA, "
_cQuery += " CT2_HIST, CT2_LOTE "
...
_cQuery += " AND (CT2_LP IN ('510','511','512','515','530','531','532','566','571','577','650')) "

_cQuery += " UNION ALL "

_cQuery += " SELECT CT2_XKEY AS CT2_KEY, CT2_XKEY AS FORNECE, "
_cQuery += " CT2_LP, CT2_ITEMD, CT2_ITEMC, CT2_CCC, CT2_CCD, CT2_VALOR, CT2_DC, CT2_DATA, "
_cQuery += " CT2_HIST, CT2_LOTE "
_cQuery += " FROM "+RetSqlName("CT2")+" "
_cQuery += " WHERE D_E_L_E_T_ = ' ' "
...
ENDIF
_cQuery += " AND CT2_DATA BETWEEN '"+Dtos(MV_PAR01)+"' AND '"+Dtos(MV_PAR02)+"' "
_cQuery += " AND (CT2_LOTE = '009800') "


TcQuery _cQuery New Alias "TRT"
//ACRESCENTADO CODIGO 577 NA QUERY PARA DESDOBRAMENTO
...
While !TRT->(EOF())
	...
	_cFornece	:= ""
	If Len(Alltrim(TRT->FORNECE)) == 19
		_cFornece	:= Substr(TRT->FORNECE,12,6)
	ElseIf Len(Alltrim(TRT->FORNECE)) == 22
		_cFornece	:= Substr(TRT->FORNECE,15,6)
	ElseIf Len(Alltrim(TRT->FORNECE)) == 23
		_cFornece	:= Substr(TRT->FORNECE,16,6)
	ElseIf Len(Alltrim(TRT->FORNECE)) == 26
		_cFornece	:= Substr(TRT->FORNECE,19,6)
	ElseIf Len(Alltrim(TRT->FORNECE)) == 25
		_cFornece	:= Substr(TRT->FORNECE,16,6)
	ElseIf Len(Alltrim(TRT->FORNECE)) == 28
		_cFornece	:= Substr(TRT->FORNECE,19,6)
	ElseIf Len(Alltrim(TRT->FORNECE)) == 35
		_cFornece	:= Substr(TRT->FORNECE,26,6)
	ElseIf Len(Alltrim(TRT->FORNECE)) == 38
		_cFornece	:= Substr(TRT->FORNECE,29,6)
	ElseIf Len(TRT->FORNECE) == 201
		_cFornece	:= Substr(TRT->FORNECE,03,6)
	EndIf

	If _cFornece < MV_PAR09 .OR. _cFornece > MV_PAR10
		TRT->(DBSKIP())
		LOOP
	ENDIF
	...
	DO CASE
		CASE  ALLTRIM(TRT->CT2_LP) == "650"
			...
			lTitSE2 := FPESKEY(_cPrefixo,_cNumero," ","NF ",_cFornece,_cLOja,TRT->CT2_LP)
	        cTIPO    := "NF "
	        cNUMERO  := _cNumero
		...
		CASE ALLTRIM(TRT->CT2_LP) $ "532"
			...
			EndIf
			_cNumero := iif(Len(_cNumero)<9, _cNumero+Space(3), _cNumero)
			lTitSE2 := FPESKEY(_cPrefixo,_cNumero,_cParcela,_cTipo,_cFornece,_cLOja,TRT->CT2_LP)
	        cTIPO    := _cTipo
	        cNUMERO  := _cNumero

		CASE ALLTRIM(TRT->CT2_LP) $ "510/511/512/515/532/566/571/577"
			//XFILIAL("SE2")+E2_PREFIXO+E2_NUM+E2_PARCELA+E2_TIPO+E2_FORNECE+E2_LOJA
			_cFilial 	:= SUBS(TRT->CT2_KEY,01,2)
			_cPrefixo 	:= SUBS(TRT->CT2_KEY,03,3)
			_cNumero	:= ""
		...
		CASE ALLTRIM(TRT->CT2_LP) $ "530/531"
			...
			EndIf
			_cNumero := iif(Len(_cNumero)<9, _cNumero+Space(3), _cNumero)
			lTitSE2 := FPESKEY(_cPrefixo,_cNumero,_cParcela,_cTipo,_cCliFor,_cLOja,TRT->CT2_LP)
	        cTIPO    := _cTipo
	        cNUMERO  := _cNumero
		CASE ALLTRIM(TRT->CT2_LOTE) == "009800"
			 _dEmissao 		:= StoD(Substr(TRT->CT2_KEY,09,08))		//lTitSE2[1][1]  SE2->E2_EMISSAO
			 _dVencimento 	:= StoD(Substr(TRT->CT2_KEY,09,08))		//lTitSE2[1][2]  SE2->E2_VENCREA
			 _cNomFor 		:= Substr(TRT->CT2_KEY,29,30)				//lTitSE2[1][3]  SE2->E2_NOMFOR
			 _cNumBor 		:= ""										//lTitSE2[1][4]  SE2->E2_NUMBOR
			 _cNumAP 		:= ""										//lTitSE2[1][5]  SE2->E2_NUMAP
			 _cNumChq 		:= ""										//lTitSE2[1][6]  SE2->E2_NUMBCO
			 _dBaixa 		:= StoD(Substr(TRT->CT2_KEY,09,08))		//lTitSE2[1][7]  SE2->E2_BAIXA
			 _dDtConc 		:= StoD(Substr(TRT->CT2_KEY,09,08))		//lTitSE2[1][8]   dDtConc
			 _cFornece 		:= Substr(TRT->CT2_KEY,03,06)	   			//lTitSE2[1][9]  SE2->E2_FORNECE
			 _cLoja 		:= "01"										//lTitSE2[1][10]  SE2->E2_LOJA
		     _cHistor 		:= TRT->CT2_HIST							//lTitSE2[1][10]  SE2->E2_HIST

		     _cNumero 		:= Substr(TRT->CT2_KEY,17,06)
		     _cTipo 		:= Substr(TRT->CT2_KEY,01,02)
			lTitSE2			:= {}
			AAdd(lTitSE2,{_dEmissao,_dVencimento,_cNomFor,_cNumBor,_cNumAP,_cNumChq,_dBaixa,_dDtConc,_cFornece,_cLoja,_cHistor})			

	        cTIPO    := alltrim(_cTipo)
	        cNUMERO  := alltrim(_cNumero)
	...
	If !Empty(lTitSE2[1][2])     
		...
		dbSelectArea('SA2')
		dbSetORder(1)
		If dbSeek(xFilial('SA2')+lTitSE2[1][9]+lTitSE2[1][10])
			__cNomFor := Substr(A2_NOME,1,30)
		Else
			__cNomFor := lTitSE2[1][9]
		EndIf
		RecLock("TRB",.T.)
		TRB->CONTA   := pConta
...
While !TRB->(Eof())
	...
	@ nLin, 065+3 PSay TRB->DEBITO  Picture "@E 9,999,999.99"
	@ nLin, 081+3 PSay TRB->CREDIT  Picture "@E 9,999,999.99"
	@ nLin, 097+3 PSay TRB->BORDERO
	@ nLin, 107+3 PSay TRB->NUMAP
	@ nLin, 119+3 PSay TRB->VENCREA
	@ nLin, 130+3 PSay TRB->HIST
	...
	If _Conta <> TRB->CONTA
		If nTotDGer+nTotCGer > 0
			...
			@ nLin, 064+3 PSay nTotDGer  Picture "@E 99,999,999.99"
			@ nLin, 080+3 PSay nTotCGer  Picture "@E 99,999,999.99"
			nLin:=nLin+1
		EndIf
	EndIf
...
@ nLin, 064+3 PSay nTotDFin  Picture "@E 99,999,999.99"
@ nLin, 080+3 PSay nTotCFin  Picture "@E 99,999,999.99"
nLin++
@ nLin, 000 PSay Replicate("-", 132)
	...
	IF SD1->(DBSEEK(xFilial("SD1")+pNUMERO+pPREFIXO+pFORNECE+pLOJA))
		...
		IF SF4->(DBSEEK(xFilial("SF4")+SD1->D1_TES))
			IF ALLTRIM(SF4->F4_DUPLIC) == "N"
				...
				AAdd(alRet,{SD1->D1_EMISSAO,SD1->D1_EMISSAO,Substr(SA2->A2_NOME,1,30)," "," "," ",CTOD("//"),CTOD("//"),SD1->D1_FORNECE,SD1->D1_LOJA,})
				RestArea(cAlias)
				Return(alRet)
			ENDIF
		ENDIF
...
SE2->(DBSETORDER(1))
SE2->(DBGOTOP())
IF SE2->(DBSEEK(XFILIAL("SE2")+pPREFIXO+pNUMERO+pPARCELA+pTipo+pFORNECE+pLOJA)) .AND. ALLTRIM(pLP) $ "510/511/512/515/530/531/532/566/571/577/650"
	...
	ENDIF
ELSEIF SE2->(DBSEEK(XFILIAL("SE2")+pPREFIXO+pNUMERO+"A"+pTipo+pFORNECE+pLOJA)) .AND. ALLTRIM(pLP) $ "510/511/512/515/530/531/532/566/571/577/650"
	DbSelectArea("SE2")
	Do While !EOF() .AND. SE2->(XFILIAL("SE2")+pPREFIXO+pNUMERO+pTipo+pFORNECE+pLOJA) == SE2->(E2_FILIAL+E2_PREFIXO+E2_NUM+E2_TIPO+E2_FORNECE+E2_LOJA)
		If SE2->E2_VENCREA >= MV_PAR03 .AND. SE2->E2_VENCREA <= MV_PAR04
			IF ALLTRIM(pLP) $ "566/571"
				DBSELECTAREA("SEF")
				SEF->(DBSETORDER(3))
...
ElseIf Len(Alltrim(TRT->CT2_KEY)) == 26 // Titulos Novos 9 posicoes E2_NUM
	_cQuery += " AND SUBSTRING(CT2_KEY,19,6) BETWEEN '"+MV_PAR09+"' AND '"+MV_PAR10+"' "
EndIf
