function displayFields(form,customHTML){ 
	...
	if(!informacoesColaboradorRH["erro"]){
		...
		form.setValue("txtMatSuperior", informacoesColaboradorRH["Mat_sup"] );
		form.setValue("txtMatAtf", informacoesColaboradorRH["Resp_Atf"] );		
...
function consultarInformacoesRHPorMatriculaUsuario(matricula){
	...
	try {
		...
		informacoes["Mat_sup"] = xmlDoc.ACOLS.item.CAMP07;
		informacoes["empresa"] = xmlDoc.ACOLS.item.CAMP08;
		informacoes["filial"]  = xmlDoc.ACOLS.item.CAMP09;
		informacoes["Resp_Atf"] = xmlDoc.ACOLS.item.CAMP10;
