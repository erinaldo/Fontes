...
Private lEnd
Private cArq	    := ""
Private _cBancoSE5	:= "", _cAgSE5 := "", _cContSE5	:= ""
Private cFLIncons	:= ""
...
If lRet
	...
	cFLIncons := AllTrim(Soma1(SuperGetMV("CI_REFFLCP",.T.,"0000000")))
	PutMV("CI_REFFLCP",cFLIncons)
	...
	For _nI := 1 to len(oGetDados1:aCols)
		If oGetDados1:aCols[_nI,5] > 0
			...
			PAB->(MSUNLOCK())
			//U_GravaPAJ(1, oGetDados1:aCols[_nI][5])
		EndIf
		...
		For _nW := 1 to len(oGetDados1:aCols)
			If oGetDados1:aCols[_nW,5] > 0
				...
				If _lAchou
					...
					U_GravaPAJ(1, oGetDados1:aCols[_nW][5])
					RecLock("PAB",.F.)
					...
					SE5->(DbSetOrder(7))
					SE5->(DbGotop())
					If SE5->(DbSeek(xFilial("SE5")+PAB->(PAB_PREFIX+PAB_NUM+" "+PAB_TIPO+PAB_CLIFOR+PAB_LOJA)))
						While SE5->(!EOF()) .AND. SE5->(E5_PREFIXO+E5_NUMERO+E5_PARCELA+E5_TIPO+E5_CLIFOR+E5_LOJA) == PAB->(PAB_PREFIX+PAB_NUM+" "+PAB_TIPO+PAB_CLIFOR+PAB_LOJA)
							If SE5->E5_MOTBX <> 'RES' .AND. SE5->E5_SITUACA <> 'C' .AND. SE5->E5_RECONC <> 'x' .AND. SE5->E5_VALOR == _nSldAnt
								...
								RecLock("SE5",.F.)
								SE5->E5_SITUACA := "C"
								SE5->E5_XREFFLC := Iif(Len(AllTrim(SE5->E5_XREFFLC))>0,AllTrim(SE5->E5_XREFFLC)+"|","")+cFLIncons
								SE5->(MsUnLock())
					...
					cQuery += "SET E5_XREFFLC='"+cFLIncons+"' "
					cQuery += "WHERE D_E_L_E_T_='' "
					cQuery += "AND E5_PREFIXO='"+PAB->PAB_PREFIX+"' "
					cQuery += "AND E5_NUMERO='"+PAB->PAB_NUM+"' "
					cQuery += "AND E5_PARCELA=' ' "
					cQuery += "AND E5_TIPO='"+PAB->PAB_TIPO+"' "
					cQuery += "AND E5_FORNECE='"+PAB->PAB_CLIFOR+"' "
					cQuery += "AND E5_LOJA='"+PAB->PAB_LOJA+"' "
					cQuery += "AND E5_MOTBX='RES' "
					cQuery += "AND E5_DATA='"+DtoS(dDatabase)+"' "
					cQuery += "AND R_E_C_N_O_>="+AllTrim(Str(_nRecSE5))+" "
					tCSqlexec(cQuery)
					...
					If PAB->PAB_SALDO > 0
						...
						cQuery += "AND E5_DATA='"+DtoS(dDatabase)+"' "
						cQuery += "AND E5_BANCO='"+_cBancoSE5+"' "
						cQuery += "AND E5_AGENCIA='"+_cAgSE5+"' "
						cQuery += "AND E5_CONTA='"+_cContSE5+"' "
						cQuery += "AND E5_DATA='"+DtoS(dDatabase)+"' "
						cQuery += "AND R_E_C_N_O_>="+AllTrim(Str(_nRecSE5))+" "
						tCSqlexec(cQuery)
						/*
