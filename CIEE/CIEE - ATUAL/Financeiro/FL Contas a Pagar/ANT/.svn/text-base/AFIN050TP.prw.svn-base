					...
					aVetor :={	{"E2_PREFIXO"	,PA9->PA9_PREFIX,Nil},;
								...
								{"E2_LOJA"  	,PA9->PA9_LOJA	,Nil},;
					   			...
					   			{"AUTVLRPG"		,SE5->E5_VALOR ,Nil}}
					   			 ...
					   			 //autvlrpg  			
					MSExecAuto({|x,y| Fina080(x,y)},aVetor,5) //Cancelamento da Baixa do Titulo,inclui a data da baixa do titulo no SE2,e gera registro do titulo no SE5    
...
_cQR := " SELECT PAB_CLIFOR, PAB_NATURE, PAB_PREFIX, PAB_NUM, PAB_LOJA, PAB_VALOR, ED_CODIGO, ED_DESCRIC "
...
_cQR += " PAB_LOJA = '"+PA9->PA9_LOJA+"' AND "	
...
DbselectArea("QRY")
While QRY->(!EOF())   
	AADD(aPags,{.f.,QRY->PAB_NATURE,QRY->PAB_VALOR,QRY->PAB_VALOR,QRY->ED_DESCRIC,0})
	QRY->(DBSKIP())
Enddo 
...
IF SELECT("TCIEE") > 0
    M->PA9_NATURE := "8.88.88"
Else
	M->PA9_NATURE := "9.99.99"
Endif
...
If alltrim(PA9->PA9_PREFIX) == "FLI"
	_cQry := "SELECT * "
	_cQry += "FROM "+RetSQLname('PAC')+" "
	_cQry += "WHERE D_E_L_E_T_ = '' "
	_cQry += "AND PAC_NUMORI = '"+ALLTRIM(PA9->PA9_NUM)+"' "
	_cQry += "ORDER BY PAC_NUMORI "
	_cQry := ChangeQuery(_cQry)
	DbUsearea(.T., 'TOPCONN', TCGENQRY(,,_cQry),'PACTM1',.F.,.T.)  
...
/*
	...
	_cQry += "FROM "+RetSQLname('PAC')+" "
	_cQry += "WHERE D_E_L_E_T_ = '' "
	_cQry += "AND PAC_NUMFLI = '"+ALLTRIM(PA9->PA9_NUM)+"' "
	_cQry += "ORDER BY PAC_NUMORI "
	_cQry := ChangeQuery(_cQry)
	DbUsearea(.T., 'TOPCONN', TCGENQRY(,,_cQry),'PACTM2',.F.,.T.)  
	...
	_cQry	:= "SELECT R_E_C_N_O_, * FROM "+RetSQLname('SE5')+" "
	_cQry	+= "WHERE D_E_L_E_T_ = '' "
	_cQry	+= "AND E5_HISTOR LIKE '%"+ALLTRIM(PA9->PA9_NUM)+"%' "
	_cQry := ChangeQuery(_cQry)
	DbUsearea(.T., 'TOPCONN', TCGENQRY(,,_cQry),'SE5TMP',.F.,.T.)  

	DbSelectarea("SE5TMP")
	DbGotop()
	_nValCanc	:= 0
	_cChave		:= SE5TMP->(E5_PREFIXO+E5_NUMERO+E5_PARCELA+E5_CLIFOR+E5_LOJA)
	Do While !EOF()
	
		If _cChave <> SE5TMP->(E5_PREFIXO+E5_NUMERO+E5_PARCELA+E5_CLIFOR+E5_LOJA)
			_nValCanc	:= 0
		EndIf
		_nValCanc+= SE5TMP->E5_VALOR

		DbSelectArea("SE5")
		DbGoto(SE5TMP->R_E_C_N_O_)
		RecLock("SE5",.F.)
		SE5->E5_SITUACA := "C"
		MsUnLock()
		DbSelectarea("SE5TMP")
...
DBSETORDER(3)
IF DBSEEK(xFilial("PAB")+PA9->PA9_PREFIX + PA9->PA9_NUM + PA9->PA9_FORNEC)
	While PAB->(!EOF()) .AND. PA9->PA9_PREFIX + PA9->PA9_NUM + PA9->PA9_FORNEC == PAB->PAB_PREFIX + PAB->PAB_NUM + PAB->PAB_CLIFOR
		RECLOCK("PAB",.F.)
		DBDELETE()
		MSUNLOCK()
		PAB->(DBSKIP())
...
DbSelectArea("PAA")
PAA->(DbSetOrder(3))
If Dbseek(xFilial("PAA")+ PA9->PA9_NUM + PA9->PA9_PREFIX + PA9->PA9_FORNEC + PA9->PA9_LOJA)
	Do While PAA->(!EOF()) .and. PA9->(PA9_NUM+PA9_PREFIX+PA9_FORNEC+PA9_LOJA) == PAA->(PAA_TITULO+PAA_PREFIX+PAA_FORNEC+PAA_LOJA)
		RecLock("PAA",.F.)
		DbDelete()
		MsUnLock()
		PAA->(DbSkip())
...
If M->PA9_RATEIO == 'N'
	...
	IF DBSEEK(xFilial("CT1")+M->PA9_CTADEB)
		If !EMPTY(CT1->CT1_NATURE)
			_cNatureza	:= "8.88.88" //CT1->CT1_NATURE
		Else
			_cNatureza	:= "8.88.88"
		ENdif	
	ENDIF 
