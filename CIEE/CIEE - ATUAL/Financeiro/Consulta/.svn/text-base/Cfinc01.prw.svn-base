...
//		If dbSeek(xFilial("SEF")+SE2TMP->E2_PREFIXO+SE2TMP->E2_NUM+SE2TMP->E2_PARCELA+SE2TMP->E2_TIPO,.F.)
	...
	If !Empty(SE2TMP->F1_DOC)
		_cUserLG    := FWLeUserlg("SE2TMP->F1_USERLGI")//PATRICIA FONTANEZI - 09/10/12 - ALTERAÇÃO DA FUNÇÃO EMBARALHA
		_cUsuario1  := Subs(_cUserLG,1,15)
	Else
		_cUserLG    := FWLEUSERLG("SE2TMP->E2_USERLGI")//PATRICIA FONTANEZI - 09/10/12 - ALTERAÇÃO DA FUNÇÃO EMBARALHA
		_cUsuario1  := Subs(_cUserLG,1,15)
	EndIf
...
//	aAdd(aPags,{SE2TMP->E2_TIPO+"/"+SE2TMP->E2_NUM, SE2TMP->E2_VENCREA, SE2TMP->E2_NOMFOR, SE2TMP->E2_VALOR, SE2TMP->E2_VALLIQ, SE2TMP->E2_NUMBOR, SE2TMP->E2_NUMAP, _cCheque, _cFl, SE2TMP->E2_NATUREZ, SE2TMP->E2_EMISSAO, SE2TMP->E2_BAIXA, _dConc, _cBanco, _cAgencia, _cConta, _cUsuario1, SE2TMP->E2_HIST })
	aAdd(aPags,{SE2TMP->E2_TIPO+"/"+SE2TMP->E2_NUM, SE2TMP->E2_VENCREA, SE2TMP->E2_RAZSOC, SE2TMP->E2_VALOR, SE2TMP->E2_VALLIQ, SE2TMP->E2_NUMBOR, SE2TMP->E2_NUMAP, _cCheque, _cFl, SE2TMP->E2_NATUREZ, SE2TMP->E2_EMISSAO, SE2TMP->E2_BAIXA, _dConc, _cBanco, _cAgencia, _cConta, _cUsuario1, SE2TMP->E2_HIST, SE2TMP->E2_USUALIB })
...
If Len(aPags) > 0
	...
	@ 05,05 LISTBOX oLbx1 FIELDS HEADER "Titulo","Vencimento","Fornecedor","Valor Titulo","Valor Liquido","Bordero","AP","Cheque","FL","Natureza","Emissao","Baixa","Concilacao" ,"Banco","Agencia","Conta","Usuario","Historico","Usuario Liberacao"  SIZE 310, 130 OF oDlg PIXEL //;
	
	
	oLbx1:SetArray(aPags)
	oLbx1:bLine := { || {aPags[oLbx1:nAt,1], aPags[oLbx1:nAt,2], aPags[oLbx1:nAt,3], Transform(aPags[oLbx1:nAt,4],"@EZ 999,999,999.99"), Transform(aPags[oLbx1:nAt,5],"@E 999,999,999.99"), aPags[oLbx1:nAt,6], aPags[oLbx1:nAt,7], aPags[oLbx1:nAt,8], aPags[oLbx1:nAt,9], aPags[oLbx1:nAt,10], aPags[oLbx1:nAt,11], aPags[oLbx1:nAt,12],aPags[oLbx1:nAt,13],aPags[oLbx1:nAt,14],aPags[oLbx1:nAt,15],aPags[oLbx1:nAt,16],aPags[oLbx1:nAt,17],aPags[oLbx1:nAt,18],aPags[oLbx1:nAt,19]  } }
	oLbx1:nFreeze  := 1
   ...
   //	DEFINE BUTTON FROM 145, 224 TYPE 7  ENABLE OF oDlg ACTION U_CFINR032()
	@ 145,220 BUTTON "Imp LOG" SIZE 28,11 ACTION (U_CFINR032())
