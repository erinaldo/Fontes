...
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºALTERAÇÕES³ DESCRIÇÃO                                                  º±±
±±º 06/06/13 ³ Alterado o campo que identifica o tipo da Natureza (1=Sin- º±±
±±º          ³ tetica/2=Analitica) de ED_CLASSE para ED_TIPO (padrão)     º±±
±±º          ³ by Daniel G.Jr.                                            º±±
...
//	If SED->ED_CLASSE == "1" //Se for analitica nao traz		// Retirado em 06/06/2013
	If SED->ED_TIPO   == "2" //Se for analitica nao traz		// Incluido em 06/06/2013
		DbSelectArea("SED")
		SED->(DbSkip())
		Loop
	EndIf
	...
	Do While !EOF()
		...
		Do Case
			Case mv_par02 == 1
				...
				_dDat	:= FLX->FLX_DT1
			Case mv_par02 == 2
				If ALLTRIM(FLX->FLX_NATU) == "0.X" .or. ALLTRIM(FLX->FLX_NATU) == "000" .or. ALLTRIM(FLX->FLX_NATU) == "0.00"// "0.00"
					_nVal 	:= FLX->FLX_VL1
					_dDat	:= FLX->FLX_DT2
				Else
		...
		If ALLTRIM(FLX->FLX_SUPORC) == "50010102" .or. ALLTRIM(FLX->FLX_NATU) == "2.05"// "2.05" //Natureza grupo DESCONTO
			DbSelectArea("FLX")
			FLX->(DbSkip())
		EndIf
