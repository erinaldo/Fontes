...
While !Eof()
	...
	While !Eof() .And. _cE5_ORDEM == E5_ORDEM
		...
		While &(cCondWhile)
			...
			_cDescNat := POSICIONE("SED",1,xFilial("SED")+iif(E5_MOEDA == "RC",E5_NATREC,E5_NATUREZ),"ED_DESCRIC")
			...
			If Empty(_cNatureza)
				SZZ->ZZ_NATUREZ	:= "9.99.99"
				SZZ->ZZ_DESCNAT	:= "SEM NAT"
			Else
				SZZ->ZZ_NATUREZ	:= iif(TMP->E5_MOEDA == "RC",TMP->E5_NATREC,TMP->E5_NATUREZ)
