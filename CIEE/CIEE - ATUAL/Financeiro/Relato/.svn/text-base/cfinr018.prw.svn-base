...
AADD(_aPerg,{cPerg,"06","D -1               ?","","","mv_ch6","C",01,0,0,"C","","mv_PAR06","Sim","","","","","Não","","","","","","","","","","","","","","","","","","","","",""})
//Incluído parâmetro 6 (D -1) SSI 14/0081
...
LOCAL aStruct := {}
Local cFilterUser  
Private cDtParamDe	:= "" //Variavel para controle do parâmetro D -1.
Private cDtParamAte	:= "" //Variavel para controle do parâmetro D -1.
...
//Tratamento incluso para respeitar competencias de alguns bancos que trabalham com D -1 para emissão do extrato.
If MV_PAR06 == 1 
   cDtParamDe	:= MV_PAR03 - 1
   CDtParamDe	:= DataValida(cDtParamDe, .F.)
   CDtParamAte	:= MV_PAR04 - 1
   CDtParamAte	:= DataValida(cDtParamAte, .F.)
elseif MV_PAR06 == 2
   cDtParamDe	:= MV_PAR03
   CDtParamAte	:= MV_PAR04
EndIf   
...
If !Empty(mv_par01)
	...
	dbSeek(xFilial("SZG")+_cBanco+_cAgencia+_cConta+DTOS(cDtParamDe), .T.)
	If Eof()
...
//		If SZG->ZG_EMISSAO < mv_par03 .Or. SZG->ZG_EMISSAO > mv_par04
		If SZG->ZG_EMISSAO < cDtParamDe .Or. SZG->ZG_EMISSAO > cDtParamAte
			dbSelectArea("SZG")
			dbSkip()
			Loop
		EndIf
