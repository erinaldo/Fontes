...
RptStatus({|lEnd| f710ImpCRT(@lEnd,wnRel,cString,Tamanho)},titulo)
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function f710ImpCRT(lEnd,wnRel,cString,Tamanho)
Local lFirst := .T.
SetRegua(mv_par04)
For i := 1 to mv_par04  // Qtde de vias
	IncRegua()
	...
	EndIF
	li := 0
	Fa710ICRT(@lFirst, @lEnd,wnRel,cString,Tamanho)
Next i
Return
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function FA710ICRT(lFirst, lEnd,wnRel,cString, Tamanho)
...
Do While !Eof() .And. cFilial == EA_FILIAL .And. EA_NUMBOR <= mv_par02
	...
	//IncRegua()
	//If Empty(EA_NUMBOR) .or. SEA->EA_CART != "P"			
	//	dbSkip()
	//	Loop
	//Endif
	...
	IF li > 80 .or. lNew
		fr710Cabcrt(SEA->EA_MODELO, nTipo, Tamanho, @lFirst)
		m_pag++
		lNew := .F.
	End
...
//		@ li,11 PSAY SEA->EA_PARCELA
	...
	Elseif SEA->EA_MODELO $ "CC/01/03/04/05/07/08/10"
		dbSelectArea("SA6")
		dbSeek(cFilial+SA2->A2_BANCO)         
		_cNREDUZ := SA6->A6_NREDUZ
		If Empty(_cNREDUZ)
		   dbSelectArea("SZ1")
//		   If dbSeek(cFilial+SA2->A2_BANCO)
		...
		If ! lAbatimento
			@li,13  PSAY SubStr(SE2->E2_NATUREZ, 1, 10)
			@li,23  PSAY SubStr(_cNREDUZ, 1, 12)
//			@li,36  PSAY SA2->A2_BANCO + " " + ALLTRIM(SA2->A2_AGENCIA) + IIF(!EMPTY(SA2->A2_DVAG),"-"+SA2->A2_DVAG,SA2->A2_DVAG) +" " + SA2->A2_NUMCON
			@li,36  PSAY SE2->E2_BANCO + " " + ALLTRIM(SE2->E2_AGEFOR) + IIF(!EMPTY(SE2->E2_DVFOR),"-"+SE2->E2_DVFOR,SE2->E2_DVFOR) +" " + ALLTRIM(SE2->E2_CTAFOR)
		...
		Else
			If !lAbatimento
				@li,115 PSAY SE2->E2_SALDO - nAbat - SE2->E2_DECRESC + SE2->E2_SDACRESC Picture "@E 999,999,999.99"
				nAbat := 0
				nTotValor := nTotValor + SE2->E2_SALDO - SE2->E2_DECRESC + SE2->E2_SDACRESC 
			Else
				nTotValor := nTotValor - SE2->E2_SALDO 
	...
	If cNumBor != SEA->EA_NUMBOR
		...
		If li != 80
			...
			If cModelo $ "CH/02"
				...
				li++
				//	@li, 0 PSAY OemToAnsi(STR0009) + cNumConta + OemtoAnsi(STR0010)+" "+ DtoC(SE2->E2_VENCREA) //"DEBITANDO EM NOSSA CONTA CORRENTE NO DIA "
				li++
				//	@li, 0 PSAY OemToAnsi(STR0010)  //"PELO VALOR ACIMA TOTALIZADO."
				//	li++
				@li, 0 PSAY SM0->M0_NOMECOM  // Nadia
			Elseif cModelo $ "CT/30"
				@li, 0 PSAY OemToAnsi(STR0008) + OemToAnsi(STR0009) + cNumConta + OemtoAnsi(STR0010)+" "+ DtoC(SE2->E2_VENCREA)
				li++
				//  @li, 0 PSAY OemToAnsi(STR0009) + cNumConta + OemtoAnsi(STR0010)+" "+ DtoC(SE2->E2_VENCREA)
				//	li++
				@li, 0 PSAY SM0->M0_NOMECOM  // Nadia
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function fr710cabCrt(cModelo, nTipo, Tamanho, lFirst)
Local cCabecalho
Local cTexto
...
Cabec1 := cCabecalho
li := Cabec7crt(Titulo,Cabec1,NomeProg,tamanho,Iif(aReturn[4]==1,GetMv("MV_COMP"),;
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Static Function cabec7crt(cTitulo,cCabec1,cNomPrg,nTamanho,nChar,aCustomText,lFirst)
...
@ 01,00 PSAY __PrtLogo()
@ 02,00 PSAY __PrtFatLine()
//@ 04,00 PSAY __PrtLeft(SM0->M0_NOME)
//@ 04,00 PSAY __PrtCenter("BORDERO PARA PAGAMENTO DE COMPROMISSOS ")
//@ 04,00 PSAY __PrtRight("EMISSÃO :")
//@ 04,00 PSAY __PrtRight(dtoc(dDataBase))	// Data EmissÆo
@ 04,00 PSAY SUBS(SM0->M0_NOME,1,20)
@ 04,55 PSAY "BORDERO PARA PAGAMENTO DE COMPROMISSOS "
@ 04,114 PSAY "EMISSÃO : "+dtoc(dDataBase)	// Data EmissÆo
//@ 05,00 PSAY __PrtCenter(Tabela("58", SEA->EA_MODELO))  //descri‡Æo do tipo de bordero
//@ 05,00 PSAY __PrtRight("RELAÇÃO :")
//@ 05,00 PSAY __PrtRight(SEA->EA_NUMBOR)      	// Nro do bordero
@ 05,55 PSAY Tabela("58", SEA->EA_MODELO)      //descri‡Æo do tipo de bordero
@ 05,114 PSAY "RELAÇÃO : "+SEA->EA_NUMBOR      	// Nro do bordero
...
/*
li += 4
@li, 00 PSAY OemtoAnsi(_cDet1)
li += 1
@li, 00 PSAY OemtoAnsi(_cDet2)
*/
...
/*
_cCab  := "|  CONTA DEVEDORA   *    CR     *   CONTA CREDORA   *    CR     * HISTORICO *  COMPLEMENTO DO HISTORICO  *         VALORES         |"
_cDet1 := "|                   *           *                   *           *           *                            *                         |"
_cDet2 := "|___|___|___|___|___*___|___|___*___|___|___|___|___*___|___|___*___|___|___*____________________________*____________|____________|"
_cRod  := "|                               *                               *           *                            *            |            |"
li += 4
@li, 00 PSAY __PrtThinLine(); li++ 

@li, 00 PSAY OemtoAnsi(_cCab); li++
@li, 00 PSAY __PrtThinLine(); li++
For J := 1 to 10
	@li, 00 PSAY OemtoAnsi(_cDet1); li++
	@li, 00 PSAY OemtoAnsi(_cDet2); li++
Next J
li++
@li, 00 PSAY __PrtThinLine(); li++
@li, 00 PSAY OemtoAnsi(_cRod); li++
@li, 00 PSAY __PrtThinLine() 
*/
