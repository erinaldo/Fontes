...
If _lTodos .Or. _lSo_Outros
	...
	U_IMP_QUERY()
	IF MV_PAR16 == 1
		GRVPRELAN(_aConCon) // (CFINR004) Grava Lancamentos contabeis (ExecAuto atraves de Array)
//		GRVLANDBF(_aConCon) // (CFINR030) Grava Lancamentos contabeis (ExecAuto atraves de Arquivo DBF)
//		GRVLANMAN(_aConCon) // (CFINR031) Grava Lancamentos contabeis (Grava o CT2 atraves de RecLock .T. com base no array)
	ENDIF
...
While !EOF()
	...
	While !EOF().And. (QUERY->Z5_BANCO == _cBanco) .And. (QUERY->Z5_AGENCIA == _cAgencia);
		...
		While !EOF() .And. (QUERY->Z5_BANCO == _cBanco) .And. (QUERY->Z5_AGENCIA == _cAgencia);
			...
			If mv_par16 == 1
				dbSelectArea("SZ5")
				DbSetOrder(3) // Alterado dia 05/05/08 pelo analista Emerson (antes estava com o indice 1. Verificamos o erro e a necessidade de olhar o valor tambem.
				If dbSeek(xFilial("SZ5")+QUERY->(STR(QUERY->Z5_VALOR,14,2)+DTOS(Z5_LANC)+Z5_PRESTA+Z5_UNIDADE+Z5_DOC), .F.)
		...
		If AllTrim(_cUnidade)<>"SAO PAULO"
			Do Case
				Case ALLTRIM(_cGrupo) == "C"
					_cContaD   :="    31808"
				Case  ALLTRIM(_cGrupo) == "D"
					_cContaD   :="    31803"
				Case  ALLTRIM(_cGrupo) == "E"
					_cContaD   :="    31806"
				Case  ALLTRIM(_cGrupo) == "F"
					_cContaD   :="    31801"					
				Case  ALLTRIM(_cGrupo) == "G"
					//_cContaD   :="    33509"		   
					_cContaD   :="2121111"	//Patricia Fontanezi 20/09/2012 - alteração do Conta Contábil								
				OtherWise
					_cContaD   :="    31805"
			EndCase
			...
			@ nLin, 009 PSay "|"  
			If _cContaD	<> "2121111"             // PATRICIA FONTANEZI - 20/09/2012
		   		@ nLin, 011 PSay _cContaD+" "+_cCR  
		 	Else
		 		@ nLin, 011 PSay _cContaD
		 		_cCR	:= " "
		 	Endif
			@ nLin, 027 PSay "|"
			@ nLin, 040 PSay "|"
	...
	If	_nDevedora == 0  // SAO PAULO
		...
		If AllTrim( _cPresta)=="SABESP"
			@ nLin, 011 PSay "    31801 00914"
			_cDeved := "31801"
			_cCR := "00914"
		ElseIf AllTrim( _cPresta)=="ELETROPAULO"
			@ nLin, 011 PSay "    31803 00914"
			_cDeved := "31803"
			_cCR := "00914"
		ElseIf AllTrim( _cPresta)=="COMGAS"
			@ nLin, 011 PSay "    31806 00914"
			_cDeved := "31806"
			_cCR := "00914"
		ElseIf AllTrim( _cPresta)=="SEM PARAR"
			@ nLin, 011 PSay "    32020 00030"
			_cDeved := "32020"
			_cCR := "00030"
		ElseIf AllTrim( _cPresta)=="NET"
			@ nLin, 011 PSay "    31904 00030"
			_cDeved := "31904"
			_cCR := "00030"
		Else
			@ nLin, 011 PSay "       2121111"
			_nDevedora:=_nDevedora+Val("2121111")
			_cDeved := "2121111"
		EndIf
...
STATIC FUNCTION GRVLANMAN(pMatriz)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³CFINR004  ºAutor  ³Microsiga           º Data ³  11/09/10   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
STATIC FUNCTION GRVLANDBF(pMatriz)
...
If Len(pMatriz) <= 0 // Se o array nao tiver conteudo sai da rotina de contabilizacao
	RestArea(_axArea)
	Return
EndIf

_cArq	:= "\DATA\DM_CTA_"+cempant
If !File(_cArq)
	// Define a estrutura do arquivo de trabalho.
	_aEstrut := {	{"TMP_DEBITO"	,"C", 20, 0, OemToAnsi("Cta Debito"		)},;
					{"TMP_CREDIT"	,"C", 20, 0, OemToAnsi("Cta Credito"	)},;
					{"TMP_TOTAL"	,"N", 20, 2, OemToAnsi("Total"	 		)},;
					{"TMP_DM"		,"C", 10, 0, OemToAnsi("DM"		 		)},;
					{"TMP_UNID"		,"C", 15, 0, OemToAnsi("Unidade" 		)},;
					{"TMP_CR"		,"C", 10, 0, OemToAnsi("CR"		 		)},;
					{"TMP_DATA"		,"D", 08, 0, OemToAnsi("Data"	 		)},;
					{"TMP_HIST"		,"C", 20, 0, OemToAnsi("Historico"		)}}

	// Cria o arquivo de trabalho.
	dbCreate(_cArq,_aEstrut) // Fluxo Detalhado
	dbUseArea(.T., "DBFCDX", _cArq, "TMP", .F., .F.)
	IndRegua("TMP", _cArq, "TMP_DM+TMP_DEBITO",,,OemToAnsi("Criando indice..."), .T.)

	For Ix := 1 To Len(pMatriz)
		RecLock("TMP",.T.)
		TMP_DEBITO	:= pMatriz[Ix,1]
		TMP_CREDIT	:= pMatriz[Ix,2]
		TMP_TOTAL	:= pMatriz[Ix,3]
		TMP_DM		:= pMatriz[Ix,4]
		TMP_UNID	:= pMatriz[Ix,5]
		TMP_CR		:= pMatriz[Ix,6]
		TMP_DATA	:= iif(empty(pMatriz[Ix,7]),CTOD("//"),pMatriz[Ix,7]) 
		TMP_HIST	:= pMatriz[Ix,8]
		MsUnLock()
	Next Ix
Else
	DbSelectArea("TMP")
...
/*
//Caso seja necessario fechar o arquivo e deletar o DBF utilizar comandos abaixo
DbCloseArea("TMP")
Ferase(_cArq+"DBF")
Ferase(_cArq+OrdBagExt()) //deleta arquivo de indice.
*/
...
Do While !EOF()
	...
	IF !Empty(TMP->TMP_DEBITO) .and. Empty(TMP->TMP_CREDIT)
		_cDC 	:= "1"
		_cITEMD	:= TMP->TMP_DEBITO
		_cITEMC := ""
	ELSEIF Empty(TMP->TMP_DEBITO) .and. !Empty(TMP->TMP_CREDIT)
		_cDC 	:= "2"
		_cITEMD := ""
		_cITEMC := TMP->TMP_CREDIT
	ELSE
		_cDC 	:= "3"
		_cITEMD := TMP->TMP_DEBITO
		_cITEMC := TMP->TMP_CREDIT
	...
	AADD(aItem,{	{"CT2_FILIAL"	,xFilial("CT2")									, NIL},;
					...
					{"CT2_ITEMD"	,_cITEMD										, NIL},;
					{"CT2_ITEMC"	,_cITEMC										, NIL},;
					{"CT2_CCD"		, alltrim(TMP->TMP_CR)							, NIL},;
					{"CT2_CCC"		, "" 											, NIL},;
					{"CT2_DCD"		, "" 											, NIL},;
					{"CT2_DCC"		, "" 											, NIL},;
					{"CT2_VALOR"	, Round(TMP->TMP_TOTAL,nDecs)					, NIL},;
					{"CT2_HP"		, ""											, NIL},;
					{"CT2_HIST"		, alltrim(TMP->TMP_DM)+SPACE(2)+Dtoc(MV_PAR02)	, NIL},;
					{"CT2_TPSALD"	, "9"											, NIL},;
					{"CT2_ORIGEM"	, "932 "+alltrim(TMP->TMP_DM)					, NIL},;
					{"CT2_MOEDLC"	, "01"											, NIL},;
					{"CT2_EMPORI"	, Substr(cNumEmp,1,2)							, NIL},;
					{"CT2_ROTINA"	, "CFINR004"									, NIL},;
					{"CT2_LP"		, ""											, NIL},;
