...
cNatDePara:=''
If Padr('33080101',15) >= mv_par03 .And. PAdr('33080101',15) <= mv_par03
	cNatDePara:=Padr('6.08.01',15)
EndIf
...
If !Empty(cNatDePara)                                                     
	cQuery += " AND ( (E5_NATUREZ BETWEEN '"+mv_par03+"' AND '"+mv_par04+"' ) or E5_NATUREZ='"+cNatDePara+"' )"
Else
...
cQuery += "       (E5_TIPODOC IN ('TR','TE') AND E5_MOEDA IN ('TR','TE')                                         AND E5_RECONC <> ' ' )   )"
If mv_par05 == 1
	cQuery += " AND E5_NATREC <>  ' '   "
ElseIf mv_par05 == 2
	cQuery += " AND E5_NATREC =   ' '   "
...
*/
			...
			If E5_RECPAG == "P"
				...
				If ('33080101' == Alltrim(E5_NATUREZ)) .Or. (Alltrim(cNatDePara) == Alltrim(E5_NATUREZ) )
					_nVlFluxo+=nValor 
				EndIf
...
If aRecon[2][SAIDA] <> 0
	...
	//Grava_Fluxo(aRecon[2][SAIDA])
	Grava_Fluxo(_nVlFluxo)
...
_cChave:="CFINR016"+Space(_nTam)+DTOS(dDataBase)                                

If !(dbSeek(xFilial("PAH")+_cChave))
	RecLock("PAH", .T.)
		PAH->PAH_FILIAL := xFilial("PAH")            
		PAH->PAH_ORIGEM := "CFINR016"
