...
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Daniel GJr³ - Conforme solicitação da OS nr.13/0047, foi incluido o ti-³±±
...
±±³Daniel G.J³ Inclusão dos modelos	97 e 98 no lay-out do relatório, conf.³±±
...
Do While SEA->(!Eof()) .And. cFilial == SEA->EA_FILIAL .And. SEA->EA_NUMBOR <= mv_par02
	...
	SE2->(dbSeek( cFilial+SEA->EA_PREFIXO+SEA->EA_NUM+SEA->EA_PARCELA+SEA->EA_TIPO+SEA->EA_FORNECE+cLoja ))
	_nRegSE2:=RECNO()
	While SE2->(!Eof()) .And. ;
	...
	End
	If !lBaixa
		If (SE5->(dbSeek( cFilial+"BA"+SE2->E2_PREFIXO+SE2->E2_NUM+SE2->E2_PARCELA+SE2->E2_TIPO+DtoS(SE2->E2_BAIXA)+SE2->E2_FORNECE)))
			While SE5->(!Eof()) .and. ;
				E5_FILIAL	== cFilial			.and. ;
				E5_TIPODOC	== "BA"            	.and. ;
				E5_PREFIXO	== SE2->E2_PREFIXO 	.and. ;
	...
	End
	dbSelectArea( "SEF" )
	If (SEF->(dbSeek( cFilial+SE5->E5_BANCO+SE5->E5_AGENCIA+SE5->E5_CONTA+SE5->E5_NUMCHEQ)))
		lCheque := .t.
	End
	dbSelectArea( "SA2" )
...
//	Elseif SEA->EA_MODELO $ "CP/31/99/88"		// alterado em 26/03/2013 conf. OS 13/0047
	Elseif SEA->EA_MODELO $ "CP/31/97/98/99/88"		// alterado em 08/10/2013 conf. SSI 13/0217
		If ! lAbatimento
			@li,13+3 PSAY SubStr(SE2->E2_NATUREZ, 1, 10)
			@li,23+3 PSAY SubStr(SA2->A2_NOME,1, 30)
		EndIf
	...
	Else
		If ! lAbatimento
			@li,13+3 PSAY SubStr(SE2->E2_NATUREZ, 1, 10)
			@li,23+3 PSAY SubStr(SA2->A2_NOME,1, 33)
		EndIf
		dbSelectArea( "SA6" )
		dbSeek( cFilial+SE2->E2_PORTADO)
		dbSelectArea( "SEA" )
	...
	DO WHILE SEA->(!Eof()) .And. cFilial == SEA->EA_FILIAL .And. SEA->EA_NUMBOR <= mv_par02 ;
		.and. (Empty(SEA->EA_NUMBOR) .or. SEA->EA_CART != "P")
		dbSkip( )
	ENDDO
