...
While !Eof()
	...
	dbSelectArea("SE8")
	dbSetOrder(1)
	dbSeek(xFilial("SE8")+_cBanco+_cAgencia+_cConta+Dtos(mv_par03))
	If Found() .and. SE8->E8_FLAG == "X"
	...
	While .T. .AND. _nContador <= 30 
		If dbSeek(xFilial("SZG")+_cBanco+_cAgencia+_cConta+DTOS(MV_PAR03-_nContador))
			Do While (xFilial("SZG")+_cBanco+_cAgencia+_cConta+DTOS(MV_PAR03-_nContador)) == SZG->(ZG_FILIAL+ZG_BANCO+ZG_AGENCIA+ZG_CONTA+DTOS(ZG_EMISSAO))
				If SZG->ZG_VALFIN >= 0			
...
_cChave:="CFINR017"+Space(_nTam)+DTOS(mv_par03)                                

If !(dbSeek(xFilial("PAH")+_cChave))
	RecLock("PAH", .T.)
		PAH->PAH_FILIAL := xFilial("PAH")            
		PAH->PAH_ORIGEM := "CFINR017"
