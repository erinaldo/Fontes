...
If mv_par01 == mv_par02
   titulo := "Fechamento do Recebimento de Contribuicoes - F I B A - "+dtoc(mv_par01)
Else
   titulo := "Fechamento do Recebimento de Contribuicoes - F I B A - "+dtoc(mv_par01)+" a "+dtoc(mv_par02)
EndIf   
			...
			Else
				If TMP->(dbSeek(_cChave, .F.))
					If QUERY->E5_PREFIXO $ TMP->ZD_SIST
						If TMP->ZD_TIPO == "O" .Or. "999999" $ SED->ED_FECHA // Tipo Outros
							RecLock("TMO", .T.)
		...
		While !EOF() .And. _cPag == TMP->ZD_PAGINA
			...
			If TMP->ZD_TIPO <> "O"
				...
				If TMP->ZD_TIPO == "T"
					...
					EndIf
				Else
					@ Val(TMP->ZD_LINHA), Val(TMP->ZD_COLUNA)    PSay LEFT(TMP->ZD_DESC,30)
					If TMP->ZD_TIPO == "S" .Or. TMP->ZD_TIPO == "G"
						@ Val(TMP->ZD_LINHA), Val(TMP->ZD_COLUNA)+32 PSay TMP->ZD_VALOR + TMP->ZD_DIFERE  Picture "@E 999,999,999,999.99"
					Else
						@ Val(TMP->ZD_LINHA), Val(TMP->ZD_COLUNA)+32 PSay TMP->ZD_VALOR                   Picture "@E 999,999,999,999.99"
					EndIf
				EndIf
			EndIf
			dbSelectArea("TMP")
