...
WHILE (cTab)->(!EOF())  
	...
	IF EMPTY(aCab)                 
    	...
    	aTexto := {}
    	aTexto := StrTokArr((cTab)->ZH_OBS,CRLF,.T.)   
		...
		aRod:= {"Condição de Pagamento",; 
				...
				"Local de Entrega",;
				SUBSTR(TRIM((cTab)->ZH_LOCAL),1,80),;
				aTexto,;   
				"Supervisão",;
	...
	aTxtEsp1:= StrTokArr(SUBCARAC(1,(cTab)->C7_DESCRI),CRLF,.T.) 
  	...
  	IF nUsado == 1 
  		aTxtEsp1 := StrTokArr(SUBCARAC(2,(cTab)->B1_ESPEC),CRLF,.T.) // StrTokArr(strTran(strTran((cTab)->B1_ESPEC,"’","'"),"–","|"),CRLF,.T.)
