...
While (cAlias)->(!Eof())
	...
	If Date()>=dDtRef15                                                                                              
		If Day(Date())>15 .And. lNaoEnviou//Só envia a partir do dia 15 do mes e somente 1 vez por mes
			EnvWFJob(4,(cAlias)->R_E_C_N_O_,15)                                                                        
	...
	ElseIf Date()>=dDtRef08//8 oprque é 5 dias a partir do ultimo envio (com 3 dias)
	   	EnvWFJob(4,(cAlias)->R_E_C_N_O_,8)
	ElseIF Date()>=dDtRef03//3 dias em diante
	   	EnvWFJob(4,(cAlias)->R_E_C_N_O_,3)
	EndIf
...
ElseIf _cId==4//Oficializada e sem aceite
	_cCodProc:="000003"      
	_cTo:=SZP->ZP_EMAIL1+';'+SZP->ZP_EMAIL2+"; sistemas@cieesp.org.br; mauricio@cieesp.org.br"
	//_cCC:="sistemas@cieesp.org.br; mauricio@cieesp.org.br"   
	_cCC:=""   
	If nQtdDias==8
		_cTo+=';'+SZP->ZP_EMAIL3
	//ElseIf nQtdDias==10     
		//_cTo+=';'+SZP->ZP_EMAIL3+';'+SZP->ZP_EMAIL4
	EndIF
	_cAssunto:= OemToAnsi("Falta homologação da SSI número "+SZP->ZP_NRSSI+" !")
	_cHtml	 := "\workflow\WFSSIOfic.htm"
...
ElseIf _cId==4
	...
	//oHtml:ValByName("qtddias", Alltrim(STR(nQtdDias)))
EndIf
...
Else	
	...
	AADD(aMsg, " ")
	aAdd(aMsg, '<p><a href="'+'http://fenix:8081/workflow/messenger/emp' +cEmpAnt  + '/' + "000016" + '/' + alltrim(cProcess) + '.html">clique aqui</a> para Aprovação/Rejeição.</p>')
	AADD(aMsg, " ")
	AADD(aMsg, " ")
