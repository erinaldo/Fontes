...
cQuery +=        "D3_DOC, D3_COD, D3_CC, D3_CF, D3_QUANT, D3_CUSTO1, D3_EMISSAO, D3_ESTORNO "
...
For _nXX := 1 to Len(_aErroPV)
	AutoGrLog(_aErroPV[_nXX,2]+"-"+_aErroPV[_nXX,3])
Next _nXX
AutoGrLog("")
...
If _cFileog <> nil
	__CopyFile("\system\"+_cFileog,"\system\log_romaneio_"+dtos(mv_par03)+".txt")
	_cAnx :=  "\system\log_romaneio_"+dtos(mv_par03)+".txt"
	u_cieeMail(_cAnx)
	ferase("\system\log_romaneio_"+dtos(mv_par03)+".txt")
EndIf
...
//	@nLin, 66 PSay Transform(TRB->D3_CUSTO1, "@E 999,999,999.99")
	...
	If mv_par06 == 1 //Sim
		If !Empty(TRB->CTT_CODCLI)
			...
			If DbSeek(xFilial("SD3")+TRB->D3_DOC+TRB->D3_COD)
				RecLock("SD3")
				SD3->D3_OP := "S"
				MsUnLock()
			EndIf
