...
If SZO->ZO_BDI == "1"
	AADD(aButtons,{"NOTE",&("{||U_fConsulta(1)}"),"BDI"})
EndIf

If SZO->ZO_CONTATO == "1"
	AADD(aButtons,{"NOTE",&("{||U_fConsulta(2)}"),"Contato"})
EndIf

If SZO->ZO_ENTID == "1"
	AADD(aButtons,{"NOTE",&("{||U_fConsulta(3)}"),"Entidade"})
EndIf

If SZO->ZO_CARGO == "1"
	AADD(aButtons,{"NOTE",&("{||U_fConsulta(4)}"),"Cargo"})
EndIf

If SZO->ZO_CATEG == "1"
	AADD(aButtons,{"NOTE",&("{||U_fConsulta(5)}"),"Categoria"})
EndIf
...
Do Case
	Case nOpc == 1 // BDI
		...
		cQuery  += " AND PAB_CODEVE = '"+M->ZO_CODEVEN+"' "
		cQuery  += " ORDER BY PAB_CODEVE"
	Case nOpc == 2 // Contato
		_cAlias := "PAA"
		_cTexto := "Contato"
		cQuery  := "SELECT * "
		cQuery  += " FROM "+RetSQLname('PAA') "
		cQuery  += " WHERE D_E_L_E_T_ <> '*' "
		cQuery  += " AND PAA_CODEVE = '"+M->ZO_CODEVEN+"' "
		cQuery  += " ORDER BY PAA_CODEVE"
	Case nOpc == 3 // Entidade
		_cAlias := "PAD"
		_cTexto := "Entidade"
		cQuery  := "SELECT * "
		cQuery  += " FROM "+RetSQLname('PAD') "
		cQuery  += " WHERE D_E_L_E_T_ <> '*' "
		cQuery  += " AND ZP_CODEVEN = '"+M->ZO_CODEVEN+"' "
		cQuery  += " ORDER BY PAD_CODEVE"
	Case nOpc == 4 // Cargo
		_cAlias := "PAC"
		_cTexto := "Cargo"
		cQuery  := "SELECT * "
		...
		cQuery  += " WHERE D_E_L_E_T_ <> '*' "
		cQuery  += " AND PAC_CODEVE = '"+M->ZO_CODEVEN+"' "
		cQuery  += " ORDER BY PAC_CODEVE"
	Case nOpc == 5 // Categoria
		_cAlias := "SZP"
		_cTexto := "Categoria"
		cQuery  := "SELECT * "
...
DEFINE MSDIALOG oDlg TITLE _cTexto From 8,0 To 28,80 OF oMainWnd
...
*/
	...
	SZO->ZO_COND1   := M->ZO_COND1
	SZO->ZO_COND2   := M->ZO_COND2
	SZO->ZO_COND3   := M->ZO_COND3
	SZO->ZO_COND4   := M->ZO_COND4
	SZO->ZO_COND5   := M->ZO_COND5
	MsUnLock("SZO")
	End Transaction
Endif
...
_cCond1 := iif(M->ZO_COND1=="1","AND","OR")
_cCond2 := iif(M->ZO_COND2=="1","AND","OR")
_cCond3 := iif(M->ZO_COND3=="1","AND","OR")
_cCond4 := iif(M->ZO_COND4=="1","AND","OR")
_cCond5 := iif(M->ZO_COND5=="1","AND","OR")
...
If M->ZO_BDI == "1"
	cQuery  += _cCond1+" AC8_COD IN (SELECT PAB_CODBDI "
	cQuery  += "					FROM PAB020 "
	cQuery  += "					WHERE PAB_CODEVE = '"+M->ZO_CODEVEN+"') "
Else
	cQuery  += _cCond1+" AC8_COD BETWEEN '' AND 'ZZZZZZZZZZ' "
EndIf
If M->ZO_CONTATO == "1"
	cQuery  += _cCond2+" U5_CODCONT IN (SELECT PAA_CODCON "
	cQuery  += "					FROM PAA020 "
	cQuery  += "					WHERE PAA_CODEVE = '"+M->ZO_CODEVEN+"') "
Else
	cQuery  += _cCond2+" ISNULL(U5_CODCONT, '') BETWEEN '' AND 'ZZZZZZZZZZ' "
EndIf
If M->ZO_ENTID == "1"
	cQuery  += _cCond3+" ZQ_CODENT IN (SELECT PAD_CODENT "
	cQuery  += "					FROM PAD020 "
	cQuery  += "					WHERE PAD_CODEVE = '"+M->ZO_CODEVEN+"') "
Else
	cQuery  += _cCond3+" ISNULL(ZQ_CODENT, '') BETWEEN '' AND 'ZZZZZZZZZZ' "
EndIf
If M->ZO_CARGO == "1"
	cQuery  += _cCond4+" ZR_CARGO IN (SELECT PAC_CODCAR "
	cQuery  += "					FROM PAC020 "
	cQuery  += "					WHERE PAC_CODEVE = '"+M->ZO_CODEVEN+"') "
Else
	cQuery  += _cCond4+" ISNULL(ZR_CARGO,'') BETWEEN '' AND 'ZZZZZZZZZZZZZZZ' "
EndIf
If M->ZO_CATEG == "1"
	cQuery  += _cCond5+" ZY_COD IN (SELECT ZP_COD "
	cQuery  += "		FROM SZP020 "
...
If M->ZO_ENTID == "2" //Pegar todas as Entidades
	...
	EndIf
	If M->ZO_CARGO == "1"
		cQuery  += _cCond4+" U5_CARGO IN (SELECT PAC_CODCAR "
		cQuery  += "					FROM PAC020 "
		cQuery  += "					WHERE PAC_CODEVE = '"+M->ZO_CODEVEN+"') "
	Else
		cQuery  += _cCond4+" ISNULL(U5_CARGO,'') BETWEEN '' AND 'ZZZZZZZZZZZZZZZ' "
	EndIf
	If M->ZO_CATEG == "1"
		cQuery  += _cCond5+" ZY_COD IN (SELECT ZP_COD "
		cQuery  += "		FROM SZP020 "
