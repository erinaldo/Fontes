...
//NIVEL1
//Responsavel
_cQuery	:= "SELECT Z5_UNIDADE, Z7_GRUPO, CTT_EST "
_cQuery	+= " FROM "
...
_cQuery += " AND   '"+ _xFilCTT +"' = CTT_FILIAL"
...
_cQuery += " AND    Z5_UNIDADE  = CTT_MUN"
_cQuery	+= " AND    Z5_BAIXA = '' "
_cQuery	+= " AND    Z5_CONTA = 'S' "
_cQuery += " AND    Z5_NIVEL = ''"
_cQuery += " GROUP BY Z5_UNIDADE, Z7_GRUPO, CTT_EST"
...
//		_cCC	:= SZU->ZU_CC1 + ";" + SZU->ZU_CC2			// Alterado em 08/10/2013 - SSI. 13/0218
	...
	IF _cEst == "SP" .and. TMPUNI->Z7_GRUPO $ "A|C|G"
	   _cQuery += " AND Z7_GRUPO IN ('A','C','G') " 
	ELseIF _cEst == "SP" .and. !(TMPUNI->Z7_GRUPO $ "A|C|G")
       _cQuery += " AND Z7_GRUPO NOT IN ('A','C','G') " 	
	EndIf
