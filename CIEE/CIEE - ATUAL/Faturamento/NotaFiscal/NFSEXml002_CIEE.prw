...
Local aRetIRR	:= {}
Local aRetINS	:= {}
Local cMenUser :=''//Especifico CIEE 

//Especifico CIEE
cMenUser:=CRLF+CRLF+CRLF+"LEGISLACAO: IMUNIDADE TRIBUTARIA"+CRLF+CRLF
cMenUser+="  IRPJ - Decreto n.3000, de 26/03/1999 - Artigo n.170;"+CRLF
cMenUser+="  CSLL - Instrucao Normativa SRF n.390, de 30/01/2004 - Artigo n.11;"+CRLF
cMenUser+="  PIS/PASEP - Instrucao Normativa SRF n.247, de 21/11/2002 - Artigo n.47;"+CRLF 
cMenUser+="  COFINS - Lei n.12101, de 27/11/2009 - Artigo n.29"
		...
		While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			If !lNFeDesc
				If lNatOper .And. lSC5 .And. !Empty(SC5->C5_MENNOTA).And. nCont == 1
					...
					cNatOper +=cMenUser//Especifico CIEE
				EndIf
			Else 
				If lSC5 .And. !Empty(SC5->C5_MENNOTA).And. nCont == 1
					...
					cDiscrNFSe +=cMenUser//Especifico CIEE
				EndIf
			EndIf
...
For Nx := 1 to Len(aProd)   
	...
	Else
		nValLiq    := (aProd[Nx][27]) - aPisXml[3][Nx] - aCofinsXml[3][Nx]  - Iif(Len(aInssXml[3]) > 1,aInssXml[3][Nx],aInssXml[1]) - Iif(Len(aIRRFXml[3]) > 1,aIRRFXml[3][Nx],aIRRFXml[1]) - aCSLLXml[3][Nx] - Iif(Len(aIssRet[4]) > 1,aIssRet[4][Nx],aIssRet[1])
	EndIf
