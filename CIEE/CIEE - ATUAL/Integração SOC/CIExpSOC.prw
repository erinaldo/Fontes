...
Local aParambox 	:={}
Local cArq 			:='' 
Private cPathTransf	:='\arq_txt\fiscal\exportacao\'
Private cPathArq	:='\arq_txt\fiscal\transferencia\' 
...
Processa( {|| lProc:=CIExpProc(@cArq)}, "Aguarde...","Exportando NFS-e...", .F. )
...
Static Function CIExpProc(cArq)
...
cArq:= 'RPSSOC_'+DTos(Date())+StrTran(Time(),':','')+'.txt'
	...
	If !Empty((cAlsCan)->C5_XRPSSOC)
		...
		Else
			...
			Else
				...
				cMsg:='Emissao de Nota Autorizada.'
			EndIf
