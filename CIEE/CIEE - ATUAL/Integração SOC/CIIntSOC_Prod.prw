...
If Len(aArqs)>0
	...
	If !lErroEstr
		...
		If _lJob                                                                                   
			ConOut('Job Pedidos '+Dtoc(Date())+' '+ Time() + ' - Retornos da prefeitura - Inicio em 3 minutos.')
			Sleep(180000)//3 minutos                                                               
			ConOut('Job Pedidos '+Dtoc(Date())+' '+ Time() + ' - Retornos da prefeitura - INICIO.')
			fGetRetPrf()//Pega retorno da prefeitura (só no Job)                                   
			ConOut('Job Pedidos '+Dtoc(Date())+' '+ Time() + ' - Retornos da prefeitura - FIM.')			
		EndIf
	Else
		lErros2:=.T.
	EndIf
...
If !lMsErroAuto .And. _lIncNota
	...
	If dbSeek(xFilial('SF2')+PadR(_cNotaInc,Len(SF2->F2_DOC))+_cSerNF)
		fInputLog(_cRPS_SOC,'S',_cCNPJ)                                                               
		If _lJob//Faz a transmissão , quando Job.
			StartJob('u_fSendPref', GetEnvServer(), .F. , {_cSerNF,PadR(_cNotaInc,Len(SF2->F2_DOC))} )
		EndIF
	Else
...
/*---------------------------------------------------------
*
* Apos inclusao da NF transmite para a prefeitura
*
---------------------------------------------------------*/
User Function fSendPref(aParam)
...
If Empty(cIdEnt)
	Conout('u_fSendPref - cIdEnt vazio.')
	RpcClearEnv()
	Return()
EndIf
...
/*
*
* No final da leitura dos arquivos, pega retorno da prefeitura
*
*/
Static Function fGetRetPrf()
                                  
Local cAlias:=""
Local cQuery:=""
Private cEntSai:="1"//FIS022MNT1
//Os MV_PARs serao utilizados na FIS022MNT1
Private MV_PAR01:='RPS'
Private MV_PAR02:=''
Private MV_PAR03:=''

dbSelectArea('SAN')//Compatibilidade RetCodUser
cIdEnt:=StaticCall(FISA022,GetIdEnt)//Pega ID do TSS

If Empty(cIdEnt)
	Conout('u_fGetRetPrf - cIdEnt vazio.')
	Return()
EndIf

cQuery:="Select PAF_RPSSOC From "+RetSqlName('PAF')+" Where D_E_L_E_T_=' '"
cQuery+=" And PAF_PROC='S' Order By PAF_RPSSOC"
TcQuery cQuery New Alias (cAlias:=GetNextAlias())

While (cAlias)->(!Eof())
	If 	!(Substr(Time(),1,2)$"22/23/24/00/01/02/03/04/05/06/07")
	 	ConOut('Job Pedidos '+Dtoc(Date())+' '+ Time() + ' - Retornos da prefeitura - Horario limite do Job excedido.')			
	 	Exit
	EndIF	
	                    
	MV_PAR01:='RPS'
	MV_PAR02:=(cAlias)->PAF_RPSSOC
	MV_PAR03:=(cAlias)->PAF_RPSSOC
	aMonitor:={}
