...
For nPos := 1 to Len( aData )
	...
	If aData[nPos,1] == cEmpAnt
		cAux := fProcAll( aData[nPos,3], aData[nPos,1], aData[nPos,2], cEmpAnt, cFilAnt )
	Else
		cAux := StartJob("U_EST004Job", GetEnvServer( ), .T., aData[nPos,3], aData[nPos,1], aData[nPos,2], cEmpAnt, cFilAnt )
	EndIf 
	...
	For nAux := 1 to Len( aData[nPos,3] )
		( cAliasReg )->( dbGoTo( aData[ nPos, 3, nAux, 6 ] ) )
		RecLock(cAliasReg,.F.)
...
+------------------+-------------------------------------------------------------------------------+
*/             
Static Function fProcAll( aDados, cEmpDes, cFilDes, cEmpAtu, cFilAtu )
...
*/
User Function EST004Job( aData, cEmpDes, cFilDes, cEmpAtu, cFilAtu )

Local cRet 		:= ""

RpcSetType( 3 )
RpcSetEnv( cEmpDes, cFilDes, , , "EST" )
cRet := fProcAll( aData, cEmpDes, cFilDes, cEmpAtu, cFilAtu )
RpcClearEnv()
