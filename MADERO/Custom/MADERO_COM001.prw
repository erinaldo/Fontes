...
For nPosPrd := 1 to Len( aAllRegs )
	...
	If aAllRegs[nPosPrd,9] == cEmpAnt
		cAux := fProcAll( aAllRegs[nPosPrd,1], aAllRegs[nPosPrd,6], aAllRegs[nPosPrd,2], aAllRegs[nPosPrd,8], aAllRegs[nPosPrd,3], aAllRegs[nPosPrd,4] )
	Else
		cAux := StartJob("U_COM001Job", GetEnvServer( ), .T., aAllRegs[nPosPrd,1], aAllRegs[nPosPrd,6], aAllRegs[nPosPrd,2], aAllRegs[nPosPrd,8], aAllRegs[nPosPrd,3], aAllRegs[nPosPrd,4], AllTrim( aAllRegs[nPosPrd,9] ), cEmpAnt, cFilAnt)
	EndIf
	...
	For nPosFor := 1 to Len( aAllRegs )
		( cAliasReg )->( dbGoTo( aAllRegs[nPosPrd, 7] ) )
		RecLock( cAliasReg, .F. )
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
Static Function fProcAll( cCodPro, cDesc, cFilDes, cCodPrf, cFornec, cLoja, cEmpDes, cEmpAtu, cFilAtu )
...
Begin Transaction
cMsgLog += "*Verificando do cadastros relacionados na filial de destino " + cEmpDes + "/" + cFilDes + CRLF 
...
Else
    ...
    While !(cAliasSA5)->(Eof())
    		...
    		// -> Verifica se a tabela ้ vแlida
    	   	If AIA->AIA_DATDE <= dDataBase .and.  (AIA->AIA_DATATE >= dDataBase .or. Empty(AIA->AIA_DATATE)) 
				...
				If !Empty(AIA->AIA_CONDPG) .And. !SE4->(DbSeek(xFilial("SE4")+AIA->AIA_CONDPG))
					cMsgLog += "--> Erro: Condicao de pagamento cadastrada na tabela de preco " + (cAliasSA5)->A5_CODTAB + " nao cadastrada." + CRLF
					lRet    := .F.
				EndIf	                  
				...
				If !AIB->(DbSeek(AIA->AIA_FILIAL+AIA->AIA_CODFOR+AIA->AIA_LOJFOR+AIA->AIA_CODTAB+(cAliasSA5)->A5_PRODUTO))
					cMsgLog += "--> Erro: Produto nao encontrado para tabela de preco " + (cAliasSA5)->A5_CODTAB + "." + CRLF
					lRet    := .F.
				EndIf         
			EndIf                        
		ElseIf !(Empty((cAliasSA5)->A5_CODTAB))
			cMsgLog += "--> Erro: Tabela de preco " + (cAliasSA5)->A5_CODTAB + " nao encontrada para o fornecedor." +  CRLF 
			lRet    := .F.                  
		EndIf
		...
		If lRet
			...
			If !fExclEstru( cCodPro, cCodPrf, cFornec, cLoja, @cMsgLog )
				lRetProc := .F.
				DisarmTransaction( )
				Exit
	...
	/*
	// -> Verifica se houve algum procsamento
	If !lExistDado
		lRetProc := .F.
		cMsgLog  += "--> Sem dados para replicar." +  CRLF
		DisarmTransaction( )
	EndIf 				
	*/
EndIf
...
//Acrescenta o campo de mark
oTmpRes:SetFields({ ;
		{"FORNECE"	, "C", TamSx3("A5_FORNECE")[1]	, 0},;
...
//Definindo indice
oTmpRes:AddIndex( "01", { "EMPRESA", "FILIAL", "FORNECE", "LOJA", "PRODUTO", "CODPRF" } )
oTmpRes:Create( )
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณ fChkPro  บAutor  ณ Vinํcius Moreira   บ Data ณ 08/05/2018  บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ Checa produtos da estrutura.                               บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบUso       ณ                                                            บฑฑ
ฑฑศออออออออออฯออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออผฑฑ
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
Static Function fChkPro( aProdutos, cFilDes, cMsgLog )
...
For nX := 1 to Len( aProdutos )
	RecLock( cAliasRes, .T. )
		( cAliasRes )->PRODUTO 		:= aProdutos[ nX, 1 ]
		( cAliasRes )->DESCRICAO 	:= Posicione( "SB1", 1, xFilial( "SB1" ) + aProdutos[ nX, 1 ], "B1_DESC" )
		( cAliasRes )->FILIAL 		:= cFilDes
	( cAliasRes )->( MsUnlock( ) )
Next nX

( cAliasRes )->( dbGoTop( ) )
U_EST001Prep( Len( aProdutos ), @cAliasRes, .F. )

( cAliasRes )->( dbGoTop( ) )
While ( cAliasRes )->( !Eof( ) )
	If lRet .And. ( cAliasRes )->SUCESSO == 2
		lRet := .F.
	EndIf
	cMsgLog += ( cAliasRes )->MSGLOG
	( cAliasRes )->( dbSkip( ) )
EndDo
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
User Function COM001A( cAliasAux, cGrpNeg )
Local lRet		:= .F.
Local cQuery	:= ""
Local cAliasQry	:= ""
Default cGrpNeg	:= fGetGrpNeg( )

cAliasQry := u_xGetUn( cGrpNeg )
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
User Function COM001Job( cCodPro, cDesc, cFilDes, cCodPrf, cFornec, cLoja, cEmpDes, cEmpAtu, cFilAtu )

Local cRet 		:= ""

RpcSetType( 3 )
RpcSetEnv( cEmpDes, cFilDes, , , "EST" )
cRet := fProcAll( cCodPro, cDesc, cFilDes, cCodPrf, cFornec, cLoja, cEmpDes, cEmpAtu, cFilAtu )
RpcClearEnv()
...
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿
*/
Static Function fExclEstru( cCodPro, cCodPrf, cFornec, cLoja, cMsgLog )
