...
::oEventLog := oEventLog
	...
	::nLimite := SuperGetMV("MD_LIMITWS",.F.,10)
...
Local cLogAux  := "" 
	...
	If !ADK->(dbSeek(xFilial("ADK")+cFilAnt))
	   	lIsEnable := .F.
	   	cLogAux   := "Filial "+cFilAnt+" não encontrada no cadastro de unidades de negocio."
	   	::oEventLog:broken("Erro na empresa.",cLogAux,.F.)
...
Local cErrorLog := ""
	...
	::oEventLog:SetAddInfo("Pesquisando dados.",cErrorLog)
	ConOut(cErrorLog)
...
Local cErrorLog := ""
	...
	::oEventLog:SetAddInfo("Conectando.",cErrorLog)
	ConOut(cErrorLog)
	...
	If ::oConexao:isEnable()
		...
		::oEventLog:SetAddInfo("Enviando dados.",cErrorLog)
		ConOut(cErrorLog)

		If AllTrim(cXml) <> ""
		
			//salva o XML no log
			::oEventLog:setXmlEnv(cXml, len(aLote))
			...
			If ::oConexao:send( cXml )
				...
				cErrorLog:=": Processando XML..." 
					::oEventLog:SetAddInfo("Retorno dos dados.",cErrorLog)
				ConOut(cErrorLog)
			...
			Else
				...
				cErrorLog:=::oConexao:GetError() + " em " + ::oConexao:getUrl()
				::oEventLog:broken("Falha na conexao.", cErrorLog, .F.)
...
Local lOk		:= .T.
	...
	IF ::oConexao:isEnable()
		...
		Else
			//mesmo o method getConexao retornando OK
			//pode ter erro de conexao passada
			//e não será retornado em XML
			::oEventLog:broken("Erro no processo.",::oConexao:GetError() + " em " + ::oConexao:getUrl(), .F.) 
		EndIF
...
!Data              ! 21/05/2018                                              !
+------------------+--------------------------------------------------------*/
method analise(oXmlItem) class TeknisaMethodAbstract
Local lOk := .T.
