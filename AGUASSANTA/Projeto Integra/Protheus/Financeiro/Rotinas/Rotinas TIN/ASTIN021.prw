...
Static Function FS040INC(aSE1, dVenci)
	...
	Begin Transaction
		...
		cCentrCus := Posicione("SEZ",1,xFilial("SEZ")+SE1->E1_PREFIXO+SE1->E1_NUM+SE1->E1_PARCELA+SE1->E1_TIPO+SE1->E1_CLIENTE+SE1->E1_LOJA+aSE1[1,4],"EZ_CCUSTO") 
		
		If !Empty(cCentrCus)
		 	IncluiSEV(@aRatEvEz,cCentrCus,'','','',nValor,'',aSE1[1,4])
		EndIf
		
		If !Empty(aRatEvEz) 
			aadd( aArray ,{"E1_MULTNAT" , "1", Nil })
		Else
			aadd( aArray ,{"E1_MULTNAT" , "2", Nil })
		Endif		
		...
		If 	MsgNoYes("Deseja Realmente gerar o título no valor R$" + Alltrim(TRANSFORM(nValor, '@E 999,999.99')) +" ?","Atenção")
			...
			MsExecAuto( { |x,y,z,a| FINA040(x,y,z,a)} , aArray, 3,,aRatEvEz)
...
Static Function IncluiSEV(aRatEv,cCusto,cItemCta,cCLVL,cContaGer,nValRateio,cCONTA,cNatureza)
...
EndIf
aadd( aRat ,{"EV_VALOR" , nValRateio, Nil })
