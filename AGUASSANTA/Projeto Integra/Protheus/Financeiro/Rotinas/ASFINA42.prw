...
@releases   16/05/17 - Zema: 	- Baixa do titulo através do banco mutuo
                                - Gerar Movimento aglutinado no banco mutuo
/*/
//-----------------------------------------------------------------------
USER FUNCTION ASFINA42()
	...
	PRIVATE cE2_FILIAL	:= SE2->E2_FILORIG
	PRIVATE cBco		:= GETNEWPAR( "AS_BCOMUT", "MUT" )
	PRIVATE cAgenPag	:= ""
	PRIVATE cCtaPag		:= ""
	...
	PRIVATE	cE5_MOEDA	:= ""
	PRIVATE	nE5_VALOR	:= ""
	PRIVATE	cE5_NATUREZ	:= GETNEWPAR( "AS_NATMUT", "0000000004" )
	PRIVATE	cE5_BANCO	:= ""
	PRIVATE	cE5_AGENCIA	:= ""
	PRIVATE	cE5_CONTA	:= ""
	PRIVATE	cE5_BENEF	:= ""
	...
	IF cFUN == "U_FINA090" .AND. TYPE("CBCO090") <> "U"
		IF EMPTY(SE5->E5_BANCO)
			RECLOCK("SE5",.F.)
			SE5->E5_BANCO := CBCO090
			SE5->(MsUnlock())  
		ENDIF
	ENDIF	
...
//	ALERT( "CAL:" + cFUN + " FUN: " + FUNNAME() + " NUM: " + SE2->E2_NUM + " PRE: " + SE2->E2_PREFIXO + " E5_NUM: " + SE5->E5_NUMERO + " E5_PRE: " + SE5->E5_PREFIXO )
	...
	IF (SE2->E2_XBCOFIL <> cFilAnt .AND. !EMPTY(SE2->E2_XBCOFIL)) .OR. (cFun=="U_FINA090" .AND. XFILIAL("SE5") <> SE5->E5_FILIAL) // se for pagamento de mutuo
		...
		IF SE5->E5_TIPODOC = "VL"
			...
			cFilPag		:= IF(cFun=="U_FINA090",cFILAUX,SE2->E2_XBCOFIL)
			cE5_FILIAL	:= IF(cFun<>"U_FINA090",SUBSTR(SE2->E2_XBCOFIL,1,LEN(ALLTRIM(XFILIAL("SE5")))),SUBSTR(cFILAUX,1,LEN(ALLTRIM(XFILIAL("SE5")))))
			...
			cE5_BANCO	:= SE5->E5_BANCO
			cE5_AGENCIA	:= SE5->E5_AGENCIA
			cE5_CONTA	:= SE5->E5_CONTA

            DBSELECTAREA("SE5")
            // Atualiza saldo do banco de baixa
            ...
            cE5_HISTOR	:= "PGTO MUTUO: " + ALLTRIM(SM0->M0_NOMECOM)
			...
			cFilAnt := IF(cFun=="U_FINA090",cE2_FILIAL, cFILAUX)
			...
			SE5->E5_BANCO	:= cBco
			SE5->E5_AGENCIA	:= cAgenPag
			SE5->E5_CONTA	:= cCtaPag
			SE5->E5_XLOTMUT	:= cLoteMut
			SE5->E5_XBCOFIL	:= IF(cFun=="U_FINA090",cFILAUX, SE2->E2_XBCOFIL)
			SE5->( MsUnLock() )
                                  
            // Atualiza saldo do banco mutuo
			AtuSalBco(cBco,cAgenPag,cCtaPag,SE5->E5_DATA,SE5->E5_VALOR,"-")
...
STATIC FUNCTION ExecSE5()
	...
	LOCAL aFINP100		:= {}
	LOCAL nOpcPag		:= 3
	LOCAL cFILAUX		:= IF(cFun=="U_FINA090",cE2_FILIAL,cFILANT)
	LOCAL aPagar		:= {}
	LOCAL aReceber		:= {}
...
//					{"E5_DOCUMEN" 	, cE5_DOCUMEN	, NIL},;
	...
	cFILANT := IF(cFun=="U_FINA090",cFILANT,cFILAUX)
	
	// Atualiza movimentação do mutuo
	IF lRet
		U_ASFINA58(cFilPag,cFILAUX,cLoteMut, "#R", SE5->E5_VALOR ) 
	ENDIF	
