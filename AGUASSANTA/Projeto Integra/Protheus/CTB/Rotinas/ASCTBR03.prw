...
//	cQ += " AND Z6_EMP = '"+MV_PAR02+"' AND D_E_L_E_T_ <> '*' "
	...
	ELSE
		...
		WHILE XSZ6->(!EOF())
		    ...
		    IF XSZ6->Z6_EMPCONS  == XSZ6->Z6_EMPPAR
				nPosInv := nColunas
			ENDIF
			XSZ6->(DBSKIP())
		END
...
TRCell():New(oSection1,"DESCRICAO"    	,,""  ,/*Picture*/,50,.F.,)
		...
		WHILE XSZ6->(!EOF())
		    ...
		    IF CT1->CT1_CLASSE == "2" // Analํtica                                                                                          
				...
				IF SUBSTR(CT1->CT1_CONTA,1,1) < "3" 
                	DO CASE
                		CASE CT1->CT1_NORMAL == "1"
							nValcons := nValcons + (nValELI[1] - nValELI[2])
						CASE CT1->CT1_NORMAL == "2" 
							nValcons := nValcons + (nValELI[2] - nValELI[1])
					ENDCASE
				ELSE
                 	DO CASE
                		CASE CT1->CT1_NORMAL == "1"
    ...
    IF CONSO->TIPO == "2" .AND. CONSO->CONTA <> "502001001001002"  // Analํtica - Atualizar as superiores
	    ...
	    WHILE CONSO->(!EOF())      
    		...
    		FOR nConta := 1 TO nColunas
				...
				IF SUBSTR(CT1->CT1_CONTA,1,1) < "3"  // ATIVO E PASSIVO			
                    
					IF CT1->CT1_NORMAL == "1"      
						
						IF aLinha[nConta][5] == "1"
...
*/		
			...
			cCampo 	:= "ELIDB99"
			CONSO->&(cCampo) := CONSO->&(cCampo) += aLinha[Len(aLinha)][2]
		
			cCampo 	:= "ELICR99"
			CONSO->&(cCampo) := CONSO->&(cCampo) += aLinha[Len(aLinha)][3]

			IF SUBSTR(CT1->CT1_CONTA,1,1) < "3"  // ATIVO E PASSIVO			
                    
				IF CT1->CT1_NORMAL == "1"      
						
					IF aLinha[Len(aLinha)][5] == "1"

						cCampo 	:= "BAL99"
						CONSO->&(cCampo) := CONSO->&(cCampo) += aLinha[Len(aLinha)][1]
		
	                    cCampo 	:= "CONSO99"
						CONSO->&(cCampo) := CONSO->&(cCampo) += aLinha[Len(aLinha)][4]
...
// Valor da investidora

CONSO->(DBSEEK("3"))
nValInv := 0
IF nPosInv  <> 0
	cCampo 	:= "BAL"+STRZERO(nPosInv,2)
	nValInv := CONSO->&(cCampo) 
ENDIF
...
cSeq := Soma1(cSeq)    
CONSO->ORDEM := cSeq
CONSO->DESCRICAO := "RESULTADO DA INVESTIDORA"
CONSO->CONTAN6   := "**********"
CONSO->CONSO99   := nValInv
CONSO->(MsUnlock())
...
WHILE CONSO->(!EOF())                        
    ...
    IF CONSO->BAL99 <> 0 .OR. CONSO->ELIDB99 <> 0 .OR. CONSO->ELICR99 <> 0 .OR. CONSO->CONSO99 <> 0
    	lImp := .T.
    ENDIF
...
IF nOP == 1           
	...
	IF SUBSTR(CT1->CT1_CONTA,1,1) > "2"  // CONTA DEVEDORA COMO NEGATIVO SOMENTE NO RESULTADO
