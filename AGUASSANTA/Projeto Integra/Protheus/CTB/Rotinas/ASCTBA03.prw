...
IF nOpcX == 5
	IF MsgYesNo("Deseja excluir todo periodo?")
	   lPeriodo := .T.
	   lTela	:= .F.
	ENDIF
ENDIF        
...
IF lTela
	...
	@ 014, 007 MSGET oGet_Emp1 VAR cGet_Emp F3 "EMP" WHEN nOpcx == 3 SIZE 057, 010 OF oPanel1 COLORS 0, 16777215 PIXEL VALID _EMPPAR()
...
Local aParam := {}                
Local aBox   := {}    
Local cCadastro := "Cópia das Estruturas de Participação"
Local cMesOri   := ""
Local cMesDes   := ""
Local lContinua := .T.    
Local dRevOld	:= dRevisao
Local cRevOld	:= cRevisao
...
If ParamBox( aBox,cCadastro,aParam,,,,,,,,.F.,.T.)
    ...
    ELSE    
		...
		cMesOri := CTOD("01/"+SUBSTR(ALLTRIM(aParam[1]),1,2)+"/"+SUBSTR(ALLTRIM(aParam[1]),3,4))
...
//-----------------------------------------------------------------------
/*{Protheus.doc} VLDSZ2
@Valida a existência da SZ2 para data base
@param		Nenhum
@return		Nenhum
@author 	Zema
@since 		06/11/2017
...
*/
//-----------------------------------------------------------------------
USER FUNCTION VLDSZ2(pMens)
Local cQ 	:= ""
Local lRet 	:= .T. 
Local lMens := pMens

IF  TYPE("dRevisao") == "U".OR. TYPE("cInveste") == "U"
	IF(lMens,ApMsgAlert("Falha na chamada da função VLDSZ2."),"")
	RETURN(.F.)
ENDIF
                                                         
// Verifica se existe uma estrutura de participação para este investidor na data                   
cQ := "SELECT R_E_C_N_O_ AS REG FROM "+RetSqlName("SZ2")
cQ += " WHERE Z2_FILIAL = '"+XFILIAL("SZ2")+"'"
IF cInveste <> '*'
	cQ += " AND Z2_EMP    = '"+cInveste+"'"
ENDIF
cQ == " AND Z2_DTBASE = '"+DTOS(dRevisao)+"'"
cQ += " AND D_E_L_E_T_ = ' '"
TcQuery ChangeQuery(cQ) ALIAS "XSZ2" NEW
IF XSZ2->REG == 0 
	IF(lMens,ApMsgAlert("Não existe estrutura de participacao para esta empresa."),"")
	lRet := .F.
ENDIF
XSZ2->(DBCLOSEAREA())                       
