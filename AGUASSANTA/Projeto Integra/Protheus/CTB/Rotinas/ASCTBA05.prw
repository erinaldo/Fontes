...
cQ := "SELECT MAX(Z0_REVISAO) REVISAO FROM "+RetSqlName("SZ0")+" WHERE D_E_L_E_T_ = ' ' AND Z0_DTREVIS >= '"+DTOS(FirstDay(dDataBase))+"'"
TcQuery ChangeQuery(cQ) ALIAS "XREV" NEW
cRevisao := XREV->REVISAO
XREV->(DBCLOSEAREA())
...
@ 035, 003 GROUP oGroup2 TO 095, 493 PROMPT "Consolidadora" OF oPanel1 COLOR 0, 16777215 PIXEL
ASCTB05MA()
//@ 160, 004 GROUP oGroup3 TO 260, 493 PROMPT "Consolidações" OF oPanel1 COLOR 0, 16777215 PIXEL
@ 100, 004 GROUP oGroup3 TO 260, 493 PROMPT "Consolidações" OF oPanel1 COLOR 0, 16777215 PIXEL
ASCTB05MB()
						...
						FOR nY := 1 TO LEN(aCols2)
							IF !aCols2[nY][LEN(aCols2[nY])] .AND. !EMPTY(aCols2[nY][aScan(aHead2,{|x| AllTrim(x[2]) == "Z6_EMPCONS"})])
								...
								IF SZ6->(!DBSEEK(XFILIAL("SZ6")+cCodCons+oMSNewGe1:aCols[nX][aScan(aHead1,{|x| AllTrim(x[2]) == "Z6_EMP"})]+aCols2[nY][aScan(aHead2,{|x| AllTrim(x[2]) == "Z6_EMPCONS"})]+aCols2[nY][aScan(aHead2,{|x| AllTrim(x[2]) == "Z6_TPSALDO"})]))
									...
									SZ6->Z6_EMPPAR := oMSNewGe1:aCols[nX][aScan(aHead1,{|x| AllTrim(x[2]) == "Z6_EMPPAR"})]
...
AADD(aFields,"Z6_EMPPAR")
                                                            

IF nOpcX == 4 .OR. nOpcX == 3
	AADD(aAlterFields,"Z6_EMP")
	AADD(aAlterFields,"Z6_EMPPAR")	
ENDIF
...
IF nOpcX == 3 // Gera o Acols vazio
	For nX := 1 to Len(aFields)
		If DbSeek(aFields[nX])       
			//21/02/18 - Não tera controle de sequencia de consolidação
		   	IF aFields[nX] == "Z6_SEQ"   
				Aadd(aFieldFill, "001")
			ELSE                                        
...
ELSE // Atualiza o Acols com os dados existentes
	cQ := "SELECT DISTINCT(Z6_CODIGO), Z6_EMP, Z6_EMPPAR, Z6_SEQ"
	...
	WHILE QrySZ6->(!EOF())                          
		...
		Aadd(aFieldFill, POSICIONE("SM0",1,ALLTRIM(QrySZ6->Z6_EMP),"M0_NOMECOM"))
		Aadd(aFieldFill, QrySZ6->Z6_EMPPAR)		
		Aadd(aFieldFill, .F.)
		Aadd(aCols1, aFieldFill)		
		QrySZ6->(DBSKIP())  
		aFieldFill := {}
...
//oMSNewGe1 := MsNewGetDados():New( 045, 006, 150, 486, IF(nOpcX == 3 .OR. nOpcX == 4,GD_UPDATE,0), "AllwaysTrue", "AllwaysTrue",, aAlterFields,, 999, "U_CTB05MVA()", "","AllwaysTrue", oPanel1, aHead1, aCols1)
oMSNewGe1 := MsNewGetDados():New( 045, 006, 090, 486, IF(nOpcX == 3 .OR. nOpcX == 4,GD_UPDATE,0), "AllwaysTrue", "AllwaysTrue",, aAlterFields,, 999, "U_CTB05MVA()", "","AllwaysTrue", oPanel1, aHead1, aCols1)
...
IF lRet
	...
	IF READVAR() == "M->Z6_EMP" .AND. TYPE("M->Z6_EMP") <> "U"
		...
		ELSE                                
			IF EMPTY(POSICIONE("SM0",1,ALLTRIM(M->Z6_EMP),"M0_NOMECOM"))
				ApMsgAlert("Empresa não cadastrada.")
				lRet := .F.
			ELSE
				oMSNewGe1:aCols[oMSNewGe1:nAT][aScan(aHead1,{|x| AllTrim(x[2]) == "Z6_NMEMP"})] := SM0->M0_NOMECOM 
				oMSNewGe1:aCols[oMSNewGe1:NAT][aScan(aHead1,{|x| AllTrim(x[2]) == "Z6_EMP"})] := M->Z6_EMP
				cFilSet := M->Z6_EMP
			ENDIF	
		ENDIF			
	ENDIF	             
...
IF lRet
	...
	IF READVAR() == "M->Z6_EMPPAR" .AND. TYPE("M->Z6_EMPPAR") <> "U"
		IF !EMPTY(M->Z6_EMPPAR)
			IF EMPTY(POSICIONE("SM0",1,ALLTRIM(M->Z6_EMPPAR),"M0_NOMECOM"))
				ApMsgAlert("Empresa não cadastrada.")
				lRet := .F.
			ENDIF	
		ENDIF			
...
// Trata alteração da empresa consolidadora na estrutura existente

IF !EMPTY(cSetOld)
	IF cSetOld <> cFilSet                                            
	
		IF ApMsgYesNo("Confirma a alteração da consolidadora: "+cSetOld+" para: "+cFilSet)
			// Pega a sequencia atual
			cSeq := oMSNewGe1:aCols[oMSNewGe1:nAT][aScan(aHead1,{|x| AllTrim(x[2]) == "Z6_SEQ"})]
        
   			cQ := "SELECT R_E_C_N_O_ AS REGSZ6 FROM "+RetSqlName("SZ6")+" WHERE Z6_CODIGO = '"+cCodCons+"' AND Z6_EMP = '"+cSetOld+"'"
			TcQuery ChangeQuery(cQ) ALIAS "NewSZ6" NEW		
			WHILE NewSZ6->(!EOF())
				SZ6->(DBGOTO(NewSZ6->REGSZ6))
				RECLOCK("SZ6",.F.)
				SZ6->Z6_EMP := cFilSet 
				SZ6->Z6_SEQ := cSeq
				MsUnlock()
			...
			END
			NewSZ6->(DBCLOSEAREA())		
		    ApMsgInfo("Alteração da consolidadora: "+cSetOld+" para: "+cFilSet+", realizada.")
		ELSE            
			cFilSet := cSetOld
...
IF nOpcX <> 3
	...
	WHILE QrySZ6->(!EOF())                          
		...
		WHILE QrySZ6->(!EOF()) .AND. ALLTRIM(QrySZ6->Z6_EMP)==ALLTRIM(cFilSet)
			...
			AADD(aFieldFill, POSICIONE("SM0",1,QrySZ6->Z6_EMPCONS,"M0_NOMECOM"))
...
oMSNewGe2 := MsNewGetDados():New( 110, 007, 255, 486, IF(nOpcX == 3 .OR. nOpcX == 4,GD_INSERT+GD_DELETE+GD_UPDATE,0), "AllwaysTrue", "AllwaysTrue", "+Field1+Field2", aAlterFields,, 999, "U_CTB05MVB()", "", "AllwaysTrue", oPanel1, aHead2, aCols2)
...
IF lRet	 
	IF READVAR() == "M->Z6_EMPCONS" .AND. TYPE("M->Z6_EMPCONS") <> "U"
		...
		IF lRet		
			IF EMPTY(POSICIONE("SM0",1,ALLTRIM(M->Z6_EMPCONS),"M0_NOMECOM"))
				ApMsgAlert("Empresa não cadastrada.")
				lRet := .F.
			ELSE
				oMSNewGe2:aCols[oMSNewGe2:nAT][aScan(aHead2,{|x| AllTrim(x[2]) == "Z6_NMCONS"})] := SM0->M0_NOMECOM
			ENDIF	             	
		ENDIF
	ENDIF	                     
ENDIF	                     
...
If ParamBox( aBox,cCadastro,aParam,,,,,,,,.F.,.T.)
    ...
    ELSE
		...
		IF lContinua       
		    // Localizo origem
			cQ := "SELECT Z6_CODIGO, Z6_DESCRI, Z6_EMP, Z6_EMPPAR, Z6_SEQ, Z6_EMPCONS, Z6_TPSALDO, R_E_C_N_O_ AS REG  FROM "+RetSqlName("SZ6")
			cQ += " WHERE Z6_FILIAL = '"+XFILIAL("SZ6")+"' "
...
TRCell():New(oSection1,"NOME2"    ,,""              ,/*Picture*/,50,.F., {|| POSICIONE("SM0",1,ALLTRIM(IMPCONS->Z6_EMPCONS),"M0_NOMECOM")})
...
cQ := "SELECT Z6_EMP, Z6_SEQ , Z6_EMPCONS, Z6_EMPPAR, Z6_TPSALDO"
...
While !oReport:Cancel() .and. !IMPCONS->(Eof())
	...
	IF cEtapa <> IMPCONS->Z6_EMP
		...
		oReport:SkipLine()   
		oReport:PrintText("ETAPA: "+IMPCONS->Z6_SEQ+" - "+IMPCONS->Z6_EMP+" - "+POSICIONE("SM0",1,ALLTRIM(IMPCONS->Z6_EMP),"M0_NOMECOM")+" - Empresa: "+IMPCONS->Z6_EMPPAR )
		oReport:SkipLine()   
	ENDIF
