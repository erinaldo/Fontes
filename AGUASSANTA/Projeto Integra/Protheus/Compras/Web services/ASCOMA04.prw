...
WSMETHOD detalhesPedidoCompra WSRECEIVE NomeUsuario, SenhaUsuario, CNPJEmpresa, GrupoEmpresas, NumeroPedido WSSEND	ListaDetalhesPedido WSSERVICE IntegraFluig
	...
	LOCAL aFH_SM0		:= {}
	LOCAL nPos_SM0		:= 0
	//LOCAL TpDoc			:= GetMV("MV_APRPCEC") //SuperGetMV(cSIMBx, .T., "")
	LOCAL TpDoc			:= SuperGetMV("MV_APRPCEC", .T., "")
	LOCAL cTipoCR		:= IF(TpDoc==.T.,"IP","PC")
    LOCAL cAliasQry		:= ""                                     
    LOCAL cQ			:= ""
	...
	// Zema 09/03/18 - Localiza o tipo de documento na aprovação CR_TIPO - Em alguns casos está gerando como PC e outros IP 
	cQ := "SELECT CR_TIPO FROM "+RetSqlName("SCR")
	cQ += " WHERE CR_FILIAL = '"+SC7->C7_FILIAL+"' "
	cQ += " AND LEFT(CR_NUM,6) = '"+SC7->C7_NUM+"' "
	cQ += " AND D_E_L_E_T_ = ' '"
	cAliasQry 	:= GetNextAlias()
	cQ := ChangeQuery(cQ)
	dbUseArea(.T.,"TOPCONN",TCGenQry(,,cQ),cAliasQry,.F.,.T.)

	IF !EMPTY((cAliasQry)->CR_TIPO)
		cTipoCR := (cAliasQry)->CR_TIPO
	ENDIF		     
	...
	IF EMPTY(cRetExec)
		...
		::ListaDetalhesPedido[nCnt]:TipoDocumento		:= cTipoCR
...
WSMETHOD proximoAprovador WSRECEIVE NomeUsuario, SenhaUsuario, CNPJEmpresa, GrupoEmpresas, NumeroPedido, MatriculaAprovador, NivelAprovador,  TipoDocumento WSSEND ListaproximoAprovador WSSERVICE IntegraFluig
	...
	ProcLogIni( {},"ASCOMA04")
	ProcLogAtu("MENSAGEM",OemToAnsi("METHODO: proximoAprovador."),"","ASCOMA04") 

	//-----------------------------------------------------------------------
	// Autentica usuário e senha
	//-----------------------------------------------------------------------

	ProcLogAtu("MENSAGEM",OemToAnsi("Autentica usuario e senha: "),"","ASCOMA04") 
	ProcLogAtu("MENSAGEM",OemToAnsi(cNomeUsuar),"","ASCOMA04")                                                                  
	ProcLogAtu("MENSAGEM",OemToAnsi(cSenhaUsua),"","ASCOMA04")                                                                  

	IF EMPTY(cRetExec)
		PswOrder(2) 						// 2 - Nome do usuário/grupo;
		IF (  PswSeek(cNomeUsuar, .T.) ) 	// Se for pesquisar usuário informar .T., caso contrário .F. para grupos           
			IF !PswName( cSenhaUsua )
				ProcLogAtu("MENSAGEM",OemToAnsi("Não foi possível autenticação da senha"),"","ASCOMA04")                                                                  
				cRetExec := "Não foi possível efetuar a autenticação: Usuário ou senha inválidos"
			ENDIF
		ELSE	
			cRetExec := "Não foi possível efetuar a autenticação: Usuário ou senha inválidos"
			ProcLogAtu("MENSAGEM",OemToAnsi("Não foi possível autenticação do usuario"),"","ASCOMA04")                                                                  
		ENDIF	
	ENDIF
	...
	ProcLogAtu("MENSAGEM",OemToAnsi("Localiza aprovador"),"","ASCOMA04")                                                                  
	lAchouApro := .F.
	IF EMPTY(cRetExec)
		cChaveSCR	:= xFilial("SCR") + PADR(cTipoDocum, TAMSX3("CR_TIPO")[1]) + PADR(cNumeroPed, TAMSX3("CR_NUM")[1]) + PADR(cNivelApro, TAMSX3("CR_NIVEL")[1]) 

		ProcLogAtu("MENSAGEM",OemToAnsi("Chave enviada: "),"","ASCOMA04")                                                                  
		ProcLogAtu("MENSAGEM",OemToAnsi(cChaveSCR),"","ASCOMA04")                                                                  		
		...
		IF DbSEEK(cChaveSCR)
			DO WHILE !SCR->( EOF() ) .AND. SCR->(CR_FILIAL + CR_TIPO + CR_NUM + CR_NIVEL) == cChaveSCR
				IF cMatrAprov == ALLTRIM(UsrRetName(SCR->CR_USER)) .AND. SCR->CR_STATUS == "02" 
					...
					ProcLogAtu("MENSAGEM",OemToAnsi("Achou aprovador: "+cMatrAprov),"","ASCOMA04")                                                                  		
					ProcLogAtu("MENSAGEM",OemToAnsi("Dados do SCR:"),"","ASCOMA04")                                                                  		
					ProcLogAtu("MENSAGEM",OemToAnsi(SCR->(CR_FILIAL + CR_TIPO + CR_NUM + CR_NIVEL)),"","ASCOMA04")                                                                  
		...
		ELSE
			ProcLogAtu("MENSAGEM",OemToAnsi("Não localizado SCR"),"","ASCOMA04")                                                                  
		ENDIF
	ENDIF
	...
	IF EMPTY(cRetExec)
		IF lAchouApro
			...
			ProcLogAtu("MENSAGEM",OemToAnsi("Pesquisa SC7:"),"","ASCOMA04")                                                                  		
			ProcLogAtu("MENSAGEM",OemToAnsi(xFilial("SC7")+Substr(SCR->CR_NUM,1,len(SC7->C7_NUM))),"","ASCOMA04")                                                                  		
			
			dbSelectArea("SC7")
			dbSetOrder(1)
			IF !MsSeek(xFilial("SC7")+Substr(SCR->CR_NUM,1,len(SC7->C7_NUM)))
				cRetExec := "Pedido de compras " + ALLTRIM(SCR->CR_NUM) + " não encontrado"
				ProcLogAtu("MENSAGEM",OemToAnsi("SC7 não encontrado"),"","ASCOMA04")                                                                  		
			ENDIF
		ENDIF
	ENDIF
	...
	IF EMPTY(cRetExec)
		IF lAchouApro
			...
			ProcLogAtu("MENSAGEM",OemToAnsi("Pesquisa grupo de aprovação e aprovador"),"","ASCOMA04")                                                                  		
			...
			dbSelectArea("SAK")
			dbSetOrder(1)
			IF !MsSeek(xFilial("SAK")+SCR->CR_APROV)
				ProcLogAtu("MENSAGEM",OemToAnsi("Não localizado em aprovadores: "+SCR->CR_APROV),"","ASCOMA04")                                                                  		
			ENDIF

			dbSelectArea("SA2")
			dbSetOrder(1)
			IF !MsSeek(xFilial("SA2")+SC7->C7_FORNECE+SC7->C7_LOJA)
				ProcLogAtu("MENSAGEM",OemToAnsi("Não localizado fornecedor: "+SC7->C7_FORNECE+SC7->C7_LOJA),"","ASCOMA04")                                                                  		
			ENDIF

			dbSelectArea("SAL")
			dbSetorder(3)
			IF !MsSeek(xFilial("SAL")+SCR->CR_GRUPO+SCR->CR_APROV)
				ProcLogAtu("MENSAGEM",OemToAnsi("Não localizado grupo: "+SC7->C7_APROV+SAK->AK_APROSUP),"","ASCOMA04")                                                                  		
			ENDIF
			...
			IF lLibOk
				...
				ProcLogAtu("MENSAGEM",OemToAnsi("Processa liberação:"),"","ASCOMA04")                                                                  		
				ProcLogAtu("MENSAGEM",OemToAnsi("Aprovador: "+cCodLiber+" Grupo: "+cGrupo),"","ASCOMA04")                                                                  						
				
				
				Begin Transaction
					ProcLogAtu("MENSAGEM",OemToAnsi("Chamada da função  MaAlcDoc"),"","ASCOMA04")                                                                  						
					
					lLiberou := MaAlcDoc({SCR->CR_NUM,SCR->CR_TIPO,nTotal,cCodLiber,,cGrupo,,,,,"Aprovação via Fluig"},dDATABASE,4)
                    
					IF lLiberou
						ProcLogAtu("MENSAGEM",OemToAnsi("Aprovado!!!!"),"","ASCOMA04")                                                                  						
					ELSE
						ProcLogAtu("MENSAGEM",OemToAnsi("Pendente!!!!"),"","ASCOMA04")                                                                  											
					ENDIF
					...
					// Verifica se ainda tem aprovações	
					ProcLogAtu("MENSAGEM",OemToAnsi("Verifica se ainda possui aprovações"),"","ASCOMA04")                                                                  											
					...
					IF lLiberou
						...
						// Verifica se ainda tem aprovações	
						ProcLogAtu("MENSAGEM",OemToAnsi("Pedido não possui novas aprovações, liberado."),"","ASCOMA04")                                                                  											
					...
					ELSE     
						lIniciaFlu := .T.
						ProcLogAtu("MENSAGEM",OemToAnsi("Pedido possui pendências de aprovações."),"","ASCOMA04")                                                                  											
					ENDIF	
				End Transaction
			ELSE
				cRetExec := "A operação não pode ser executada agora pois o mesmo registro está em uso por outro usuário"
				ProcLogAtu("MENSAGEM",OemToAnsi("Pedido em uso, não conseguiu reservar PC."),"","ASCOMA04")                                                                  											
			ENDIF
			SC7->(MsUnlockAll())
		ELSE // não achou processo aguardando liberação 
			lIniciaFlu := .T.
			ProcLogAtu("MENSAGEM",OemToAnsi("Não achou aprovador enviado pelo FLUIG"),"","ASCOMA04")                                                                  											
			ProcLogAtu("MENSAGEM",OemToAnsi(cMatrAprov),"","ASCOMA04")                                                                  											
		ENDIF
	ENDIF
	...
	IF EMPTY(cRetExec)
		...
		IF lIniciaFlu
			...
			ProcLogAtu("MENSAGEM",OemToAnsi("Aprovações iniciadas com pendências."),"","ASCOMA04")                                                                  											

			lLibOk := SC7Lock(cNumeroPed)
			IF lLibOk
				Begin Transaction

					ProcLogAtu("MENSAGEM",OemToAnsi("Atualiza todos os itens do pedido com em aprovação:"),"","ASCOMA04")                                                                  											
					ProcLogAtu("MENSAGEM",OemToAnsi(xFilial("SC7")+Substr(cNumeroPed,1,len(SC7->C7_NUM))),"","ASCOMA04")                                                                  											
					...
					While !SC7->(Eof()) .And. SC7->C7_FILIAL+Substr(SC7->C7_NUM,1,len(SC7->C7_NUM)) == xFilial("SC7")+Substr(cNumeroPed,1,len(SC7->C7_NUM))
						...
						ProcLogAtu("MENSAGEM",OemToAnsi("Pedido/Item: "+SC7->C7_NUM+"/"+SC7->C7_ITEM),"","ASCOMA04")                                                                  											
			...
			ELSE
				cRetExec := "A operação não pode ser executada agora pois o mesmo registro está em uso por outro usuário"
				ProcLogAtu("MENSAGEM",OemToAnsi("Pedido bloqueado: "+cNumeroPed),"","ASCOMA04")                                                                  											
			ENDIF
			SC7->(MsUnlockAll())
		ENDIF
	ENDIF
	...
	IF EMPTY(cRetExec)
		...
		ProcLogAtu("MENSAGEM",OemToAnsi("Envia proximo aprovador: "+cNumeroPed),"","ASCOMA04")                                                                  											
		...
		ProcLogAtu("MENSAGEM",OemToAnsi("Localiza proximo aprovador: "),"","ASCOMA04")                                                                  											
		ProcLogAtu("MENSAGEM",OemToAnsi(cChaveSCR),"","ASCOMA04")                                                                  											
		...
		IF DbSEEK(cChaveSCR)
			DO WHILE !SCR->( EOF() ) .AND. SCR->(CR_FILIAL + CR_TIPO + CR_NUM) == cChaveSCR
				IF SCR->CR_STATUS == "02" // Aguardando Liberacao do usuario
					...
					ProcLogAtu("MENSAGEM",OemToAnsi("Proximo aprovador: "+cProxAprov),"","ASCOMA04")                                                                  											
			...
			ENDDO
        ELSE
			ProcLogAtu("MENSAGEM",OemToAnsi("Não encontrado SCR"),"","ASCOMA04")                                                                  											
		ENDIF
