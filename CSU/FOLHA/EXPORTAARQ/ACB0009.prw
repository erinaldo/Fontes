...
aadd(aStru, {"RB_FILIAL","C",02,00} )
aadd(aStru, {"RB_MAT",   "C",06,00} )
aadd(aStru, {"RB_NOME",  "C",40,00} )
aadd(aStru, {"RB_MEDMAT","C",14,00} )
...
cQuery := "SELECT RB_FILIAL, RB_MAT, RB_NOME, RB_MEDMAT, R_E_C_N_O_ as NUMREG "
cQuery += "From " + retSqlName("SRB") + " "
cQuery += "WHERE R_E_C_N_O_ IN ( "
cQuery += "SELECT MAX(R_E_C_N_O_) AS Recno FROM " + retSqlName("SRB") + " "
cQuery += "WHERE RB_MEDMAT <> ' ' AND D_E_L_E_T_ = ' ' "
cQuery += "GROUP BY RB_MEDMAT "
cQuery += "HAVING COUNT(RB_MEDMAT) > 1 ) "
...
aEval(SRb->(dbStruct()), {|z| If(z[2] # "C" .And. z[1] $ cFields, TcSetField("TRAB",z[1],z[2],z[3],z[4]), Nil)}) 
...
While !eof()
	...
	RecLock("TEMP1", .F.)
	TEMP1->NOVOMED := left(TEMP1->RB_MEDMAT,4) + cProxNum
	MsUnlock()
	
	dbSelectArea("SRB")
	dbGoto(Temp1->NUMREG)
	RecLock("SRB",.F.)
	SRb->RB_MEDMAT := TEMP1->NOVOMED
	MsUnlock()
...
nHdl := AbreHTML()
PutHtml({"Filial","Matricula","Nome","ID Unico","Registro","Novo ID", "ID Titular"}, 1, 1, nHdl)

dbSelectArea("TEMP1")
While !eof()
	cResp := Posicione("SRA", 1, TEMP1->(RB_FILIAL+RB_MAT), "RA_MEDMAT")
	PutHtml({Temp1->RB_FILIAL,TEMP1->RB_MAT,TEMP1->RB_NOME,TEMP1->RB_MEDMAT,StrZero(TEMP1->NUMREG,10),TEMP1->NOVOMED, cResp}, 2, 0, nHdl)
