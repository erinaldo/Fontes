...
User Function ConvSR8F()

 Local	oDlg	:= NIL


 Private cPerg	:= PADR("CONSR8F",LEN(SX1->X1_GRUPO))
 Private aInfo	:= {}
 Private aDepende:= {}
 Private nDepen	:= 0
 ...
 @ 095,072 BUTTON OemToAnsi(" Converte Dados" ) SIZE 55,13 ACTION Eval( { || Pergunte(cPerg,.F.) , MigraSr8F() }  )      
 @ 095,187 BMPBUTTON TYPE 2 ACTION Close(oDlg)
 ACTIVATE DIALOG oDlg CENTERED

Return( NIL )

Static function MigraSr8F()
...
//ARQUIVO DBO_HSTA.DBF
 ...
 #IFDEF WINDOWS
   Processa({|| ImpSr8F() },"Importando Hist. FИrias para o arq. SR8")

 #ENDIF        

//return
  
Static Function Impsr8F()
 ...
 DO WHILE !EOF()
   ...
   Endif      
        ...
        SR8->R8_MAT    := _Mat
        SR8->R8_DATA   := ZZ1->DTINIGOZO 
        SR8->R8_DATAINI:= ZZ1->DTINIGOZO         
		SR8->R8_DATAFIM:= ZZ1->DTFIMGOZO      
        SR8->R8_TIPO := "F"
