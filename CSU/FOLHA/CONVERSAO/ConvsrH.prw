...
User Function ConvSRH()

 Local	oDlg	:= NIL


 Private cPerg	:= PADR("CONSRH",LEN(SX1->X1_GRUPO))
 Private aInfo	:= {}
 Private aDepende:= {}
 Private nDepen	:= 0


 @ 096,042 TO 323,505 DIALOG oDlg TITLE OemToAnsi("Conversao do Hist. de FИrias")
 @ 008,010 TO 084,222
 ...
 @ 095,072 BUTTON OemToAnsi(" Converte Dados" ) SIZE 55,13 ACTION Eval( { || Pergunte(cPerg,.F.) , MigraSrh() }  )      
 @ 095,187 BMPBUTTON TYPE 2 ACTION Close(oDlg)
 ACTIVATE DIALOG oDlg CENTERED

Return( NIL )

Static function MigraSrH()
...
//ARQUIVO HSTFSRH.DBF
 ...
 #IFDEF WINDOWS
   Processa({|| ImpSrH() },"Importando SRH")

 #ENDIF        

//return
  
Static Function ImpsrH()
 ...
 DO WHILE !EOF()
   ...
   Endif            
        ...
        RECLOCK("SRH",.T.)
        SRH->RH_FILIAL := cFilDest
        SRH->RH_MAT    := _Mat
        SRH->RH_DATABAS:= ZZ1->DTINIPERAQ
 		SRH->RH_DBASEAT:= ZZ1->DTFIMPERAQ
 		SRH->RH_DFERVEN:= 30 
 		SRH->RH_DFALTAS:= ZZ1->NROFALTAS
 		SRH->RH_DFERIAS:= IIF(ZZ1->DIASABONO > 0,20,30)
 		SRH->RH_DABONPE:= ZZ1->DIASABONO
 		SRH->RH_PERC13S:= IIF(ZZ1->QUER1APARC > 0,50,00)
 		SRH->RH_DATAINI:= ZZ1->DTINIGOZO
 		SRH->RH_DATAFIM:= ZZ1->DTFIMGOZO
 		SRH->RH_DTAVISO:= (ZZ1->DTINIGOZO-30)
 		SRH->RH_DTRECIB:= (ZZ1->DTINIGOZO-4)
