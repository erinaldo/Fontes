...
*---------------------------------------------------------------------------------------------------------------
User Function CsCtbM20(_cOperP)
* Controle dos parametros para a reprocessamento automatica
* Ricardo Luiz da Rocha - Dts Consulting - 28/02/2005 GNSJC
*---------------------------------------------------------------------------------------------------------------
private _cPergDiu:=PADR("CTBM2D",LEN(SX1->X1_GRUPO)),;
        _cPergNot:=PADR("CTBM2N",LEN(SX1->X1_GRUPO)),;
        _cLogUsu:="CsCtbm20.Lo1",;
        _cLogAdm:="CsCtbm20.Lo2",;
        _cAmbiente:="PRODUCAO",;
...
if sm0->m0_codigo<>"02"
   ...
   msgbox("Esta rotina apenas podera ser executada da empresa 02 (CardSystem/Matriz)")
   return
endif
...
dbusearea(.t.,,"SXM020","SXM",.T.,.F.)
do while sxm->(!eof())
   if alltrim(sxm->xm_codigo)==if(_cDn=="dia","CTBM20DIA","CTBM20NOITE")
      sxm->(reclock(alias(),.f.))
...
*--------------------------------------------------------------------------------
USER FUNCTION INITSCARD()
*--------------------------------------------------------------------------------
Local aParams := {'02','01'}  //Coloque aqui o código da empresa e filial.
WFScheduler(aParams)
...
dbusearea(.t.,,"SXM020","SXM",.T.,.F.)
do while sxm->(!eof())
   if alltrim(sxm->xm_codigo)=="CTBM20DIA"
      _cHoraD:=sxm->xm_hrini
   elseif alltrim(sxm->xm_codigo)=="CTBM20NOITE"
...
if !_lTemDia
   ...
   sxm->(reclock(alias(),.t.))
   sxm->XM_FILIAL :=""
   sxm->XM_CODIGO :="CTBM20DIA"
   sxm->XM_NOME   :="CsCtbm20 Diurno" 
...
if !_lTemNoite
   ...
   sxm->(reclock(alias(),.t.))
   sxm->XM_FILIAL :=""
   sxm->XM_CODIGO :="CTBM20NOITE"
   sxm->XM_NOME   :="CsCtbm20 Noturno" 
   ...
   sxm->XM_HRPROX :="00:30"
   sxm->XM_AMBIENT:=upper(_cAmbiente)
   sxm->XM_ACAO   :="u_CsCtbm20('Noturno')"
   sxm->xm_ativo  :="T"
   sxm->xm_timeout:="F"
