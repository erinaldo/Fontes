...
*---------------------------------------------------------------------------------------------------------------
User Function CsCtbM07(_cOperP)
* Controle dos parametros para a consolidacao automatica
* Ricardo Luiz da Rocha - Dts Consulting - 28/02/2005 GNSJC
*---------------------------------------------------------------------------------------------------------------
private _cPergDiu:=PADR("CTBM7D",LEN(SX1->X1_GRUPO)),;
        _cPergNot:=PADR("CTBM7N",LEN(SX1->X1_GRUPO)),;
        _cLogUsu:="CsCtbm07.Lo1",;
        _cLogAdm:="CsCtbm07.Lo2",;
        _cAmbiente:="PRODUCAO",;
...
if _cOper<>nil
   ...
   if _cOper$"diurno/noturno"
      // Recalculo de saldos nas empresas
      PREPARE ENVIRONMENT EMPRESA "98" FILIAL "01"
      ...
      for _nVez:=1 to len(_vEmpresas)
          ...
          PREPARE ENVIRONMENT EMPRESA _cEmpresa FILIAL "01"
      ...
      // Consolidacao e reprocessamento de saldo na consolidadora
      PREPARE ENVIRONMENT EMPRESA "98" FILIAL "01"    
      ...
      if sx1->(Dbseek("CTB220"+"02",.f.).and.Reclock(alias(),.F.))
	     ...
	     if sx1->(Dbseek("CTB220"+"03",.f.).and.Reclock(alias(),.F.))
            ...
            sx1->(MsUnlock())
            dbselectarea("SM0")
            set filter to m0_codigo $ "02/03/04/09/98"
            SetHideInd(.T.)
...
if sm0->m0_codigo<>"98"
   ...
   msgbox("Esta rotina apenas podera ser executada da empresa 98 (Consolidadora)")
   return
endif
...
@ 100,091 To 350,520 Dialog _oBase Title OemToAnsi("Consolidacao automatica - Administracao")
...
*--------------------------------------------------------------------------------
Local aParams := {'98','01'}  //Coloque aqui o código da empresa e filial.
WFScheduler(aParams)
