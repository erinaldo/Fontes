...
*---------------------------------------------------------------------------------------------------------------
User Function CsCtbM30(_cOperP)
* Controle dos parametros para a reprocessamento automatica
* Ricardo Luiz da Rocha - Dts Consulting - 28/02/2005 GNSJC
*---------------------------------------------------------------------------------------------------------------
private _cPergDiu:=PADR("CTBM3D",LEN(SX1->X1_GRUPO)),;
        _cPergNot:=PADR("CTBM3N",LEN(SX1->X1_GRUPO)),;
        _cLogUsu:="CsCtbm30.Lo1",;
        _cLogAdm:="CsCtbm30.Lo2",;
        _cAmbiente:="PRODUCAO",;
...
if _cOper<>nil
   ...
   _lContinua:=.t.
      ...
      // Reprocessamento de saldo empresa 03
...
if sm0->m0_codigo<>"03"
   ...
   msgbox("Esta rotina apenas podera ser executada da empresa 03 (Credit & Risk/Matriz)")
   return
endif
...
dbusearea(.t.,,"SXM030","SXM",.T.,.F.)
do while sxm->(!eof())
   if alltrim(sxm->xm_codigo)==if(_cDn=="dia","CTBM30DIA","CTBM30NOITE")
      sxm->(reclock(alias(),.f.))
...
*--------------------------------------------------------------------------------
User  FUNCTION INITSCRED()
*--------------------------------------------------------------------------------
Local aParams := {'03','01'}  //Coloque aqui o código da empresa e filial.
WFScheduler(aParams)
...
dbusearea(.t.,,"SXM030","SXM",.T.,.F.)
do while sxm->(!eof())
   if alltrim(sxm->xm_codigo)=="CTBM30DIA"
      _cHoraD:=sxm->xm_hrini
   elseif alltrim(sxm->xm_codigo)=="CTBM30NOITE"
...
if !_lTemDia
   ...
   sxm->(reclock(alias(),.t.))
   sxm->XM_FILIAL :=""
   sxm->XM_CODIGO :="CTBM30DIA"
   sxm->XM_NOME   :="CsCtbm30 Diurno" 
...
if !_lTemNoite
   ...
   sxm->(reclock(alias(),.t.))
   sxm->XM_FILIAL :=""
   sxm->XM_CODIGO :="CTBM30NOITE"
   sxm->XM_NOME   :="CsCtbm30 Noturno" 
   ...
   sxm->XM_HRPROX :="00:30"
   sxm->XM_AMBIENT:=upper(_cAmbiente)
   sxm->XM_ACAO   :="u_CsCtbm30('Noturno')"
   sxm->xm_ativo  :="T"
   sxm->xm_timeout:="F"
