...
*---------------------------------------------------------------------------------------------------------------
User Function CsCtbM09(_cOperP)
* Controle dos parametros para a reprocessamento automatica
* Ricardo Luiz da Rocha - Dts Consulting - 28/02/2005 GNSJC
*---------------------------------------------------------------------------------------------------------------
private _cPergDiu:=PADR("CTBM09D",LEN(SX1->X1_GRUPO)),;
        _cPergNot:=PADR("CTBM09N",LEN(SX1->X1_GRUPO)),;
        _cLogUsu:="CsCtbm09.Lo1",;
        _cLogAdm:="CsCtbm09.Lo2",;
        _cAmbiente:="PRODUCAO",;
...
if sm0->m0_codigo<>"09"
   ...
   msgbox("Esta rotina apenas podera ser executada da empresa 09 (Institucional/Matriz)")
   return
endif
...
dbusearea(.t.,,"SXM090","SXM",.T.,.F.)
do while sxm->(!eof())
   if alltrim(sxm->xm_codigo)==if(_cDn=="dia","CTBM09DIA","CTBM09NOITE")
      sxm->(reclock(alias(),.f.))
...
*--------------------------------------------------------------------------------
USER FUNCTION INITSINST()
*--------------------------------------------------------------------------------
Local aParams := {'09','01'}  //Coloque aqui o código da empresa e filial.
WFScheduler(aParams)
...
dbusearea(.t.,,"SXM090","SXM",.T.,.F.)
do while sxm->(!eof())
   if alltrim(sxm->xm_codigo)=="CTBM09DIA"
      _cHoraD:=sxm->xm_hrini
   elseif alltrim(sxm->xm_codigo)=="CTBM09NOITE"
...
if !_lTemDia
   ...
   sxm->(reclock(alias(),.t.))
   sxm->XM_FILIAL :=""
   sxm->XM_CODIGO :="CTBM09DIA"
   sxm->XM_NOME   :="CsCtbm09 Diurno" 
...
if !_lTemNoite
   ...
   sxm->(reclock(alias(),.t.))
   sxm->XM_FILIAL :=""
   sxm->XM_CODIGO :="CTBM09NOITE"
   sxm->XM_NOME   :="CsCtbm09 Noturno" 
   ...
   sxm->XM_HRPROX :="00:30"
   sxm->XM_AMBIENT:=upper(_cAmbiente)
   sxm->XM_ACAO   :="u_CsCtbm09('Noturno')"
   sxm->xm_ativo  :="T"
   sxm->xm_timeout:="F"
