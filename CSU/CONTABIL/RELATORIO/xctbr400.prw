...
USER Function xCTBR400(	cContaIni, cContaFim, dDataIni, dDataFim, cMoeda, cSaldos,;
...
PRIVATE nSldDTransp	:= 0
	...
	If FindFunction("TRepInUse") .And. TRepInUse()
		...
		EndIf
	Else*/
		CTBR400R3() // Executa versão anterior do fonte
	//Endif
...
oSection2:SetHeaderPage(.T.)
	...
	TRCell():New(oSection2,"HISTORICO"	,"cArqTmp",STR0035,/*Picture*/,nTamHist	,/*lPixel*/,{|| Subs(cArqTmp->HISTORICO,1,30) })// "HISTORICO"	
...
oSection4 := TRSection():New( oReport,STR0046,,, .F., .F. )	//"Complemento"
	TRCell():New(oSection4,"COMP","",STR0046,/*Picture*/,17+IIF(lAnalitico,nTamHist,0)+aTamConta[1]+aTamCusto[1]+nTamItem+nTamCLVL,/*lPixel*/,/*{|| code-block de impressao }*/)	//"COMPLEMENTO"
			...
			If MsSeek(xFilial("CT2")+cArqTMP->(DTOS(DATAL)+LOTE+SUBLOTE+DOC+SEQLAN+EMPORI+FILORI),.F.)
				...
				If CT2->CT2_DC == "4"			//// TRATAMENTO PARA IMPRESSAO DAS CONTINUACOES DE HISTORICO
					While !CT2->(Eof()) .And. CT2->CT2_FILIAL == xFilial("CT2") 			.And.;
						...
						// Sergio em Jul/2008: Incluir as continuacoes de Historico. 
						// Sergio em Mai/2009: Preencher o campo ID_PROVIS com a instrucao de provisao caso o 
						//                     lancamento estiver caracterizado desta forma:
						...
						If nAxoZC > 0
						   Replace ID_PROVIS With aStrZC[nAxoZC][2]
						EndIf                                             
...
Private cPosic   := ""
	...
	While !cArqTmp->( Eof() )
		...
		// Sergio em Set/2009: Gravar os dados referentes aos eventos:
		// 1-) Inclusao    => Data + Usuario + Hora
		// 2-) Atualizacao => Data + Usuario + Hora
		If CT2->( FieldPos('CT2_X_USRI') ) > 0
            ...
            /*
			PswOrder(2)            // Ordem de Login do Usuario
			PswSeek( Left( EMBARALHA((cTmpTxt)->CT2_USERGA,1), 15 ) )
			cCpoLgA := EMBARALHA((cTmpTxt)->CT2_USERGA,1)
			dDataA  := IIF(!Empty(cCpoLgA),CTOD("01/01/96","DDMMYY") + Load2in4(Substr(cCpoLgA,16)),"")
			cNomeA  := UsrFullName( PswId() )
	        */
			cArqTmp->USERINC := (cTmpTxt)->CT2_X_USRI // Responsavel pela Inclusao    - Sergio em Set/2009
			cArqTmp->DTINC   := Stod((cTmpTxt)->CT2_X_DTIN) // Data da Inclusao       - Sergio em Set/2009
			cArqTmp->HRINC   := (cTmpTxt)->CT2_X_HRIN // Hora da Inclusao             - Sergio em Set/2009
			cArqTmp->USERUPD := (cTmpTxt)->CT2_X_USRA       // Responsavel pela Alteracao - Sergio em Set/2009
			cArqTmp->DTUPD   := Stod((cTmpTxt)->CT2_X_DTAL) // Data da Alteracao - Sergio em Set/2009
			cArqTmp->HRUPD   := (cTmpTxt)->CT2_X_HRUP // Hora da Alteracao            - Sergio em Set/2009
		EndIf
	...
	//cArquivo := cGetFile("", "Informe o Diretorio e o nome do Arquivo em DBF",,,,GETF_LOCALHARD+GETF_LOCALFLOPPY+GETF_NETWORKDRIVE)

	cArqTmp->( DbGoTop() )
	DbSelectArea("cArqTmp")
	
	Copy To &(cCmd) DELIMITED
...
aCampos :={	{ "CONTA"		, "C", aTamConta[1], 0 },;  		// Codigo da Conta
			...
			{ "HRUPD"       , "C", 08      	    , 0 },;			// Hora da Alteracao          - Sergio em Set/2009
...
cQuery  += " AND (CT2.CT2_DC = '1' OR CT2.CT2_DC = '3' OR OR CT2.CT2_DC = '4') " // Serjin
...
//MemoWrite('C:\APEXCEL\QUEURY.TXT', cQuery)
