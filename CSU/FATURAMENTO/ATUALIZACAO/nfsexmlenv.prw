...
user function nfseXMLEnv( cTipo, dDtEmiss, cSerie, cNota, cClieFor, cLoja, cMotCancela )
	...
	If cTipo == "1"
		...
		if !empty(aNota)
			...
			cString	+= servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe )
			cString	+= valores( aISSQN, aRetido, aTotal )
			cString	+= pagtos( aDupl )
			cString	+= deducoes( aProd, aDeduz )
...
/*/
//-----------------------------------------------------------------------
static function servicos( aProd, aISSQN, aRetido, cNatOper, lNFeDesc, cDiscrNFSe )
...
/*/
//-----------------------------------------------------------------------
static function deducoes( aProd, aDeduz )
	...
	for nX := 1 to len( aDeduz )
		...
		cString		+= "<valor>" + iif( aDeduz[nX][1] == "2", allTrim( convType( aDeduz[nX][8], 15, 2 ) ), "0.00" ) + "</valor>"
