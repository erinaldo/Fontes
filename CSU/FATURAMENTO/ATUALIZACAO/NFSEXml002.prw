...
//
// Funcao Especifica para carregar mensagem da NF de Servico CSU
// David Gomes Cardoso Junior - Novembro de 2010.
//
_cMesCSU := DESCCSU()
...
If cTipo == "1"
	...
	If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
		...
		While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			If lNatOper .And. lSC5 .And. !Empty(_cMesCSU)
				If Empty(cNatOper)
					cNatOper := _cMesCSU
				EndIf
			Else	
			...
			If !AllTrim(_cMesCSU) $ cMensCli
				cMensCli := AllTrim(_cMesCSU)
			EndIf
			...
			If lAglutina
				If Len(aProd) > 0			
					...
					If nX > 0						
						...
						aProd[nx][29]+=	SF3->F3_ISSSUB + SF3->F3_ISSMAT	//Valor Total de deducoes
					Else
						lAglutina := .F.
					EndIF			                                                                                                                        					
				EndIf	
	...
	If DbSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		If SF1->(FieldPos("F1_INSS"))<>0 .and. SF1->F1_INSS>0
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				If lNatOper .And. !Empty(_cMesCSU)
					If Empty(cNatOper)
						cNatOper := _cMesCSU
					EndIf
				Else	
...
If cCodMun == '1302603'
	cInscrEst:=Alltrim(POSICIONE('SA1',3,xFilial('SA1')+aDest[01],"A1_INSCR"))
	cString += '<InscricaoEstadualTomador>'+Iif(Upper(cInscrEst)=='ISENTO' .OR. Empty(cInscrEst),"0000000",cInscrEst)+'</InscricaoEstadualTomador>'	
EndIf
...
For Nx := 1 to Len(aProd)   
	...
	Else 
		cXml += '<ItemListaServico>'+ConvType2(aProd[Nx][24],5)+'</ItemListaServico>'   
	Endif
	If cCodMun $ "5201108-4104808-1302603"      
...
Return(cRetorno)
//
// Função para levar mensagem personalizada para as notas eletronicas de servico da rotina FISA022.
// Autor.: David Gomes Cardoso Junior - Novembro de 2010.
//

Static Function DESCCSU()
Local aArea:= GetArea()
Local _cDescrx := ""
Local creturn := ""
...
IF SD2->(DbSeek( xFilial( "SD2" ) + SF2->F2_DOC + SF2->F2_SERIE))
	...
	If xFilial("SF2") == "10"
		_cQuebra :=  chr(13)+chr(10)
	Elseif xFilial("SF2") == "11"
		_cQuebra :=  "\r\n"
	EndIf
	...
	SC6->(dbSeek(xFilial("SC6")+SD2->D2_PEDIDO))
		_cDescrx += Alltrim(SC6->C6_DESCRI)+_cQuebra	
	DbSelectArea("SC5")
	SC5->(DbSetOrder(1))                                                                        
	SC5->(dbSeek(xFilial("SC5")+SD2->D2_PEDIDO))
					
		_dVencto := SC5->C5_DATA1
		_cDescrx += "Vencimento  "+Dtoc(_dVencto)+_cQuebra
		_cDescrx += Alltrim(SC5->C5_COMPETE)+_cQuebra
		_cDescrx += Alltrim(SC5->C5_MENNOT2)+_cQuebra
		_cDescrx += Alltrim(SC5->C5_MENNOTA)+_cQuebra
		_cDescrx += Alltrim(SC5->C5_MENNOT3)+_cQuebra
		_cDescrx += Alltrim(SC5->C5_MENNOT4)+_cQuebra
		_cDescrx += Alltrim(SC5->C5_MENNOT5)+_cQuebra
		_cDescrx += Alltrim(SC5->C5_MENNOT6)+_cQuebra
		_cDescrx += Alltrim(SC5->C5_MENNOT7)+_cQuebra
		_cDescrx += Alltrim(SC5->C5_MENNOT8)+_cQuebra
		_cDescrx := Substr(_cDescrx,1,999)
...
While !SE1->(EOF("SE1")).And. SE1->E1_FILIAL==xFilial("SE1") .and. SE1->E1_NUM==SF2->F2_DOC .and. SE1->E1_PREFIXO==SF2->F2_SERIE
...
_nTotRet:=(_nValIrf+_nValCof+_nValPis+_nValCsl)           

/*
SD2->(DbSetOrder(3))
SD2->(DbSeek(xFilial("SD2")+SF2->F2_DOC+SF2->F2_SERIE))
SF2->(DbSetOrder(1))
SF2->(DbSeek(xFilial("SF2")+SF2->F2_NFISCAL+SF3->F3_SERIE))
SED->(DbSetOrder(1))
SED->(dbSeek(xFilial("SED")+SA1->A1_NATUREZ))
*/
_cDescrx+= "T. Retencao R$" + Str(_nTotRet,14,2) + "  IRRF-" + Str(_nValIrf,14,2)+ "  CSLL-" + Str(_nValCsl,14,2)+_cQuebra
_cDescrx+= "COFINS-"+Str(_nValCof,14,2)+"  PIS-"+Str(_nValpIS,14,2)+_cQuebra 
