...
ฑฑบRotina    ณ CSUJ00X  บAutor  ณ Ernani Forastieri  บ Data ณ  28/02/08   บฑฑ
...
If !lManual
	FWrite( nHdl_, "Entrei no RpcSetenv"+cEol )
	RpcSetType( 3 )
...
If !MayIUseCode( 'CSUJ001' + cEmpresa + cFil )
	FWrite( nHdl_, "Entrei no !MayIUseCode"+cEol )
	
	u_MsgConMon( 'Job CSUJ001 - Empresa/Filial ' + cEmpresa + cFil + ' - Ja Esta Em Andamento' ,, .F. ) //
...
If lContinua
	...
	u_MsgConMon( 'Job CSUJ001 - Empresa '  + cEmpresa + '/' + cFilant + ' - Iniciado   '  + cTitulo,.F., .F. )
...
//FWrite( nHdl_, "Linha "+AllTrim(Str(ProcLine()))+" - Inicio da execucao da funcao Ponm010"+cEol )
FWrite( nHdl_, "Linha "+AllTrim(Str(ProcLine()))+" - Inicio da execucao da funcao PonSchedule"+cEol )		
			/*
			Ponm010(;
			.T.       	,;	// 01 -> Se o "Start" foi via WorkFlow
			.T.      	,;	// 02 -> Se deve considerar as configuracoes dos parametros do usuario
			...
			*/		
		u_PonScheduler(	cEmpresa,;	//01 -> Codigo da Empresa no SIGAMAT
...
FWrite( nHdl_, "Linha "+AllTrim(Str(ProcLine()))+" - Inicio de execucao da rotina MsgConMon."+cEol )	
	...
	u_MsgConMon( 'Job CSUJ001 - Empresa '  + cEmpresa + '/' + cFilant + ' - Finalizado ' + cTitulo,.F., .F. )
...
u_EnvMail( , 'renato.carlos@csu.com.br',, ;
