...
RpcSetType(3)
RpcSetEnv("01", "HX",,,'COM',, {"SB1","SBM","SZB","SYD"}) // HX eh a filial onde o RM Solum esta Linkado
...
DEFINE MSDIALOG _oDlg TITLE OemtoAnsi("Trnasferencia Arquivos") FROM  178 , 181  TO  548 , 717  PIXEL
// Cria Componentes Padroes do Sistema
@  004 , 003  TO  042 , 267  LABEL "Operacao" PIXEL OF _oDlg
@  015 , 006  Radio oRadioGrp1 Var nRadioGrp1 Items "Enviar","Receber" 3D Size 142 , 010  PIXEL OF _oDlg
@  017 , 169  Say "Mascara" Size  022 , 008  COLOR CLR_BLACK PIXEL OF _oDlg
@  017 , 194  MsGet oEdit1 Var cMask Size  060 , 009  COLOR CLR_BLACK PIXEL OF _oDlg
@  043 , 005  Button OemtoAnsi("Selecionar ")  Size  037 , 012  PIXEL OF _oDlg ACTION( Processa({|| RunProc(1)}) )
@  043 , 052  Button OemtoAnsi("Transferir") Size  037 , 012  PIXEL OF _oDlg ACTION( Processa({|| RunProc(2)}) )
@  043 , 102  Button OemtoAnsi("Cria Pasta") Size  037 , 012  PIXEL OF _oDlg ACTION( CriaDir() )
@  043, 152 Button OemtoAnsi("Sair") Size  037, 012 PIXEL OF _oDlg ACTION( _oDlg:End() )
@  061, 003 GET oMemo1 Var cMemo1 MEMO Size  264, 118 PIXEL OF _oDlg
...
Else
	IF Len( aArquivos ) > 0
		...
		For nIxb := 1 To Len( aArquivos )
			IncProc()
			IF nRadioGrp1 == 1  // Enviar
				IF file(cDirDestino+aArquivos[nIxb])
					If !MsgNoYes("Deseja sobrescrever?")
						Ferase(cDirDestino+aArquivos[nIxb])
