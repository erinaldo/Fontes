...
If cModo = "Cons"
	...
	@ 260,350 BmpButton Type 2 Action Func_EXCEL() //FRFECHA()
...
Cria arquivo Excel
*/
Static Function Func_EXCEL()
Local cArquivo		:= "querysql.xml"
Local cPath			:= "C:\Siga\" //GetTempPath()+'querysql.xml' //GetTempPath() - pasta temp do usuario local
Local cWorkSheet	:= "Consulta SQL"
Local cTable		:= "Tabela"
Local aColunas		:= {}
Local aLinha		:= {}

Local _nI, _nY, nAux, nLin, _cString
Local oExcel

IF APMSGYESNO("Deseja exportar a query para Excel ?","ESCOLHA")
	...
	For _nI := 1 to len(aHeader)
		aadd(aColunas, aHeader[_nI,1] )
	Next _nI
	//Criando Colunas
	For nAux := 1 To Len(aColunas)
		oExcel:AddColumn(cWorkSheet,cTable, aColunas[nAux], 1, 1, .F.)
	Next nAux
	
	//Criando Linhas
	For nLin := 1 to Len(aCols)
		aLinha := array(Len(aColunas))
		_cString := "{"
		For _nY := 1 to len(aLinha)
			aLinha[_nY] := aCols[nLin,_nY]
			If _nY > 1
				_cString+=","
			EndIf
			_cString+= "aLinha["+alltrim(str(_nY))+"]"
		Next _nY
		_cString += "}"
	
		oExcel:AddRow(cWorkSheet,cTable, &(_cString) )
	Next nLin
	...
	oAbrExcel := MsExcel():New()             //Abre uma nova conexใo com Excel
	oAbrExcel:WorkBooks:Open(cPath+cArquivo)     //Abre uma planilha
	oAbrExcel:SetVisible(.T.)                 //Visualiza a planilha
	oAbrExcel:Destroy()                        //Encerra o processo do gerenciador de tarefas
