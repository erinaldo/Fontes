...
//cQry+= "     LEFT OUTER JOIN VIEW_"+RETSQLNAME("SC1")+" SC1 ON (C1_FILIAL = C7_FILIAL AND C1_NUM    = C7_NUMSC AND C1_ITEM = C7_ITEMSC AND SC1.D_E_L_E_T_ = ' ')"+CRLF
...
WHILE (cTab)->(!EOF())  
	...
	IF EMPTY(aCab)                 
    	...
    	cObsRod:= strtran((cTab)->ZH_OBS,CRLF,"") 
    	/*aTexto := StrTokArr((cTab)->ZH_OBS," ",.T.)  
    	
    	FOR cCnta:= 1 TO LEN(aTexto)
    		IF LEN(cObsRod) <= nLenObs .and. !empty(aTexto[cCnta])
    	 		cObsRod+= aTexto[cCnta]+" "
    	 	ENDIF
	  	NEXT cCnta*/ 
	...
	aTexto:={}    
  	aTxtEsp1 := StrTokArr(strTran((cTab)->B1_ESPEC,"–","|"),CRLF,.T.) 
  	
  	AADD(aTexto,{1,(cTab)->C7_DESCRI})
...
oPrint:Line(20,15,20,820) 
oPrint:Line(140,15,140,820)
oPrint:Line(20,15,140,15)
oPrint:Line(20,820,140,820)         
...
oPrint:Box(165,15,568,820,"-4")
oPrint:Line(152,40,488,40)
oPrint:Line(152,75,488,75)
oPrint:Line(152,140,488,140)

oPrint:Line(152,215,488,215)


oPrint:Line(152,510,488,510)
oPrint:Line(152,565,488,565)
oPrint:Line(152,590,488,590)
oPrint:Line(152,650,488,650)
oPrint:Line(152,705,488,705)  
oPrint:Line(152,773,488,773)
oPrint:Line(488,15,488,820)
...
nLin:= 20                   				
oPrint:Line(nLin+468,195,nLin+548,195,)
oPrint:Line(nLin+514,15,nLin+514,820,)
