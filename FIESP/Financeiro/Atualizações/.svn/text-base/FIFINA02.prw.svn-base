...
±±ºPrograma  ³ FIFINA02 ºAutor  ³TOTVS               º Data ³  28/11/11   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Funcao para Liberacao de documentos.                        º±±
±±º          ³Funcao vem do programa FICOMA01                             º±±
...
User Function FIFINA02(_nOpcao)
// _nOpcao = 1 / Consulta
// _nOpcao = 2 / Liberacao
// _nOpcao = 3 / Estornar
...
Local _aAreaCR	:= {}
Local _cDoc		:= ""
Local _nTamDoc 	:= TamSx3("ZL_NUM")[1]           
Private lCopia 	:= .F.
...
If _nOpcao == 1 // Consulta
	...
	SZL->(DbSetOrder(1))
	If SZL->(DbSeek(xFilial("SZL")+_cDoc))
		DbSelectArea("SZL")
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³fLibRN    ºAutor  ³TOTVS               º Data ³  20/07/11   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function fLibRN(cAlias,nReg,nOpcx)
Local _cCodUser	:= ""  //Codigo do usuario de retorno
Local _lEnvMail := .T.
Local _aArea	:= GetArea()
Local _aAreaSC	:= SZL->(GetArea())
...
_cDoc := Left(SCR->CR_NUM,_nTamDoc)
SZL->(DbSetOrder(1))
SZL->(DbSeek(xFilial("SZL")+_cDoc))
cGrupo	:= SZL->ZL_APROV

DEFINE MSDIALOG oDlg FROM 0,0 TO aSize[1],aSize[2] TITLE OemToAnsi("Liberacao de Req.Numerario") PIXEL
...
@ 19,45  MSGET SZL->ZL_CC      When .F. SIZE 155,9 OF oDlg PIXEL
...
If nOpc == 2 .Or. nOpc == 3               
	...
	If SZL->(DbSeek(xFilial("SZL")+_cDoc))         
		...
		// Envio de e-mail de aprovação/reprovação
		IF !Empty( _cEMail := Alltrim(UsrRetMail( SZL->ZL_USER )) )
		    _lEnvMail := .t.
		ENDIF
		...
		If lLiberou
			...
			IF _lEnvMail
				_cBody  := "Prezado(a) "+SZL->ZL_SOLICIT+Chr(13)+Chr(10)+Chr(13)+Chr(10)
				_cBody  += "Informamos que sua Solicitação ao Armazém Nr. "+SZL->ZL_NUM+" foi aprovada."
				_cBody  += Chr(13)+Chr(10)+Chr(13)+Chr(10)
				_cBody  += "Obs: "+cObs
				ACSendMail( ,,,,_cEMail,"RN "+_cDoc+" APROVADA",_cBody)
			ENDIF
		
			// Se liberou total
			SZL->(DbSetOrder(1))
			SZL->(DbSeek(xFilial("SZL")+_cDoc))
			While !Eof("SZL") .and. SZL->ZL_NUM == _cDoc
				RecLock("SZL",.F.)
				SZL->ZL_STATUS := "3" //Aprovada
				SZL->(MsUnLock())                      
				dbSkip()
			Enddo
		EndIf
		...
		If nOpc == 3
			...
			IF _lEnvMail
				_cBody  := "Prezado(a) "+SZL->ZL_SOLICIT+Chr(13)+Chr(10)+Chr(13)+Chr(10)
				_cBody  += "Informamos que sua Solicitação ao Armazém Nr. "+SZL->ZL_NUM+" foi reprovada."
				_cBody  += Chr(13)+Chr(10)+Chr(13)+Chr(10)
				_cBody  += "Obs: "+cObs
				ACSendMail( ,,,,_cEMail,"RN "+_cDoc+" REPROVADA",_cBody)
			ENDIF
	...
	Else
		ApMsgStop("RN " + _cDoc + " nao localizado !!!","Verifique")
	EndIf
EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³fEstRN    ºAutor  ³TOTVS               º Data ³  21/07/11   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
If ApMsgYesNo("Confirma estorno da liberação?","Confirmar")
	...
	If SZL->(DbSeek(xFilial("SZL")+_cDoc))
		...
		dbSelectArea("SZL")   		
		While !Eof() .and. SZL->ZL_NUM == _cDoc                                          
			nTot += SZL->ZL_VALOR			
			dbSkip()		
		Enddo
		...
		MaAlcDoc({_cDoc,"RN",nTot,,,,,1,0,},dEmissao,3)
		
		SZL->(DbSetOrder(1))
		IF SZL->(dbSeek(XFilial("SZL")+_cDoc)) .and. !Empty(cGrupo)
			MaAlcDoc({_cDoc,"RN",nTot,,,cGrupo,,1,1,dEmissao},,1)
		EndIf
		SCR->(dbSetOrder(1))        
		
		// Se gerou SCR, bloqueia solicitacao novamente.
		IF SCR->(dbSeek(xFilial("SCR")+"RN"+_cDoc))   
		    dbSelectArea("SZL")
		    While !Eof() .and. ZL_FILIAL == xFilial("SZL") .and. ZL_NUM == _cDoc		
				RecLock("SZL",.F.)
				SZL->ZL_STATUS := "2"
				SZL->(MsUnLock())                                          
				dbSkip()
		...
		EndIf
	Else
		ApMsgStop("RN " + _cDoc + " nao localizada!","Verifique")
	EndIf
EndIf
...
//If !Empty(CO1->CO1_XGRLIB)
	cTitle    := OemToAnsi("Aprovacao de RN")
	cTitDoc   := OemToAnsi("RN")
	cHelpApv  := OemToAnsi("Esta RN nao possui controle de aprovacao.")
	cNumDoc   := SZL->ZL_NUM
	cStatus   := IIF(SZL->ZL_STATUS=="2",OemToAnsi("RN LIBERADA"),OemToAnsi("AGUARDANDO LIB."))
//EndIf
...
If !Empty(cNumDoc)
	...
	#IFDEF TOP
		...
		cQuery    += "SCR.CR_NUM = '"+Padr(SZL->ZL_NUM,Len(SZL->ZL_NUM))+"' AND "
		cQuery    += "SCR.CR_TIPO = 'RN' AND "
	...
	#ELSE
		...
		dbSetOrder(1)
		MsSeek(xFilial("SCR")+cTipoDoc+Padr(SZL->ZL_NUM,Len(SZL->ZL_NUM)))
		
	#ENDIF
	
	dbSelectArea(cAliasSCR)
	
	While !Eof() .And.(cAliasSCR)->CR_FILIAL+(cAliasSCR)->CR_TIPO+Substr((cAliasSCR)->CR_NUM,1,len(SZL->ZL_NUM))== xFilial("SCR") + cTipoDoc + cNumDoc
			...
			ElseIf aHeader[nX][02] == "bCR_SITUACA"
				Do Case
					...
					Case (cAliasSCR)->CR_STATUS == "03"
						cSituaca := OemToAnsi("RN Aprovada")
					Case (cAliasSCR)->CR_STATUS == "04"
						cSituaca := OemToAnsi("RN Bloqueada")
						lBloq := .T.
