...
@Cadastro de Investidaras x Investidas x Eventos de Equivalência 
@param		Nenhum
@return		Nenhum
@author 	Zema
@since 		20/04/2016
...
@Manutenção do Cadastro de Investidaras x Investidas x Eventos de Equivalência 
@param		Nenhum
@return		Nenhum
@author 	Zema
@since 		20/04/2016
...
IF nOpcX <> 3                
	...
	cGet_Emp := SZ2->Z2_EMP
	cGet_Des := POSICIONE("SZ0",1,XFILIAL("SZ0")+SZ2->Z2_EMP,"Z0_RAZAO")
ELSE
...
SZ0->(DBSETORDER(1))         
IF !SZ0->(DBSEEK(XFILIAL("SZ0")+ALLTRIM(cGet_Emp)))
...
AADD(aFields,"Z2_CONTA")                                          
AADD(aFields,"Z2_ITEMCTA")                                          
AADD(aFields,"Z2_ACOES")
...
ELSE // Atualiza o Acols com os dados existentes
	...
	WHILE XSZ2->(!EOF())                          
		...
		Aadd(aFieldFill, POSICIONE("SZ0",1,XFILIAL("SZ0")+XSZ2->Z2_EMPPAR,"Z0_RAZAO"))
...
IF READVAR() == "M->Z2_EMPPAR" .AND. TYPE("M->Z2_EMPPAR") <> "U"      
	...
	IF (nPos := ASCAN(oMSNewGe1:aCols,{|x| ALLTRIM(x[1]) == ALLTRIM(M->Z2_EMPPAR)})) > 0 .AND. nPos <> oMSNewGe1:NAT .AND. lRet
		...
		lRet := .F.              
		oMSNewGe1:aCols[oMSNewGe1:nAT][aScan(aHead1,{|x| AllTrim(x[2]) == "Z2_CONTA"})] := SPACE(TAMSX3("Z0_CONTA")[1])		
		oMSNewGe1:aCols[oMSNewGe1:nAT][aScan(aHead1,{|x| AllTrim(x[2]) == "Z2_ITEMCTA"})] := SPACE(TAMSX3("Z0_ITEMCTA")[1])				
		oMSNewGe1:aCols[oMSNewGe1:nAT][aScan(aHead1,{|x| AllTrim(x[2]) == "Z2_NMTIDA"})] := SPACE(TAMSX3("Z2_NMTIDA")[1])
		oMSNewGe1:aCols[oMSNewGe1:nAT][aScan(aHead1,{|x| AllTrim(x[2]) == "Z2_ACOES"})] := 0
	ELSE   
		IF EMPTY(POSICIONE("SZ0",1,XFILIAL("SZ0")+ALLTRIM(M->Z2_EMPPAR),"Z0_RAZAO"))
		...
		ELSE
            ...
            ELSE                                                                                                 
				...
				oMSNewGe1:aCols[oMSNewGe1:nAT][aScan(aHead1,{|x| AllTrim(x[2]) == "Z2_CONTA"})]  := SZ0->Z0_CONTA
				oMSNewGe1:aCols[oMSNewGe1:nAT][aScan(aHead1,{|x| AllTrim(x[2]) == "Z2_ITEMCTA"})]:= SZ0->Z0_ITEMCTA
				oMSNewGe1:aCols[oMSNewGe1:nAT][aScan(aHead1,{|x| AllTrim(x[2]) == "Z2_ACOES"})]  := SZ0->Z0_QTACOES
...
WHILE XSZ1->(!EOF())             
	aInner := {}                
	AADD(aInner,IF(XSZ1->Z1_EVENTO$cEventos,.T.,.F.))
...
TRCell():New(oSection1,"CONTA"    ,,"Conta Contábil",/*Picture*/,50,.F., {|| TRB->CONTA})
TRCell():New(oSection1,"ITEM"     ,,"Item Contábil",/*Picture*/,50,.F.,  {|| TRB->ITEMCTA})
TRCell():New(oSection1,"ACOES"    ,,"Qt. Ações",/*Picture*/,15,.F., {|| cAcoes  := TRANSFORM(TRB->ACOES, "@E 9999999999.9999999")})
TRCell():New(oSection1,"QUOTAS"   ,,"Qt. Quotas",/*Picture*/,15,.F., {|| cQuotas := TRANSFORM(TRB->QUOTAS,"@E 9999999999.9999999")})
...
IF SZ2->(DBSEEK(XFILIAL("SZ2")+SUBSTR(pPante,1,9)+SUBSTR(pParti,1,9)+DTOS(dRef)))
	...
	RECLOCK("TRB",.T.)
	TRB->INVESTE   := pPante
	TRB->NOMEE     := POSICIONE("SZ0",1,XFILIAL("SZ0")+pPante,"Z0_RAZAO")
	TRB->INVESTIDA := pParti                                             
	TRB->NOMEA     := POSICIONE("SZ0",1,XFILIAL("SZ0")+pParti,"Z0_RAZAO")
