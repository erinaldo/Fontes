						...
						FOR nY := 1 TO LEN(aCols2)
							IF !aCols2[nY][LEN(aCols2[nY])] .AND. !EMPTY(aCols2[nY][aScan(aHead2,{|x| AllTrim(x[2]) == "Z6_EMPCONS"})])
								...
								IF SZ6->(!DBSEEK(XFILIAL("SZ6")+cCodCons+oMSNewGe1:aCols[nX][aScan(aHead1,{|x| AllTrim(x[2]) == "Z6_EMP"})]+aCols2[nY][aScan(aHead2,{|x| AllTrim(x[2]) == "Z6_EMPCONS"})]+aCols2[nY][aScan(aHead2,{|x| AllTrim(x[2]) == "Z6_TPSALDO"})]))
									...
									SZ6->Z6_PER    := aCols2[nY][aScan(aHead2,{|x| AllTrim(x[2]) == "Z6_PER"})]
...
ELSE // Atualiza o Acols com os dados existentes
	cQ := "SELECT DISTINCT(Z6_CODIGO), Z6_EMP, Z6_SEQ, Z6_EMPPAR"
	...
	WHILE QrySZ6->(!EOF())                          
		...
		Aadd(aFieldFill, POSICIONE("SZ0",1,XFILIAL("SZ0")+ALLTRIM(QrySZ6->Z6_EMP),"Z0_RAZAO"))
		Aadd(aFieldFill, QrySZ6->Z6_EMPPAR)		
		Aadd(aFieldFill, POSICIONE("SZ0",1,XFILIAL("SZ0")+ALLTRIM(QrySZ6->Z6_EMPPAR),"Z0_RAZAO"))				
		Aadd(aFieldFill, .F.)
		Aadd(aCols1, aFieldFill)		
		QrySZ6->(DBSKIP())  
		aFieldFill := {}
...
IF lRet
	...
	IF READVAR() == "M->Z6_EMP" .AND. TYPE("M->Z6_EMP") <> "U"
		...
		ELSE                                
			IF EMPTY(POSICIONE("SZ0",1,XFILIAL("SZ0")+ALLTRIM(M->ZL_EMP),"Z0_RAZAO"))
...
IF lRet
	...
	IF READVAR() == "M->Z6_EMPPAR" .AND. TYPE("M->Z6_EMPPAR") <> "U"
		IF EMPTY(M->Z6_EMPPAR)
			...
			lRet := .F.
		ELSE
			IF EMPTY(POSICIONE("SZ0",1,XFILIAL("SZ0")+ALLTRIM(M->ZL_EMPPAR),"Z0_RAZAO"))
				ApMsgAlert("Empresa não cadastrada.")
				lRet := .F.
			ELSE
				oMSNewGe1:aCols[oMSNewGe1:nAT][aScan(aHead1,{|x| AllTrim(x[2]) == "Z6_NMPAR"})] := SZ0->Z0_RAZAO			
			ENDIF	                                                                                            
		ENDIF	
	ENDIF	             
...
IF nOpcX <> 3
	...
	WHILE QrySZ6->(!EOF())                          
		...
		WHILE QrySZ6->(!EOF()) .AND. ALLTRIM(QrySZ6->Z6_EMP)==ALLTRIM(cFilSet)
			...
			AADD(aFieldFill, POSICIONE("SZ0",1,XFILIAL("SZ0")+QrySZ6->Z6_EMPCONS,"Z0_RAZAO"))
			AADD(aFieldFill,QrySZ6->Z6_TPSALDO)  
			AADD(aFieldFill,QrySZ6->Z6_PER)				                                         
			Aadd(aFieldFill, .F.)
...
IF lRet	 
	IF READVAR() == "M->Z6_EMPCONS" .AND. TYPE("M->Z6_EMPCONS") <> "U"
		...
		IF lRet 
			...
			IF oMSNewGe2:aCols[oMSNewGe2:NAT][aScan(aHead2,{|x| AllTrim(x[2]) == "Z6_EMPCONS"})] ==  oMSNewGe1:aCols[oMSNewGe1:NAT][aScan(aHead1,{|x| AllTrim(x[2]) == "Z6_EMP"})]
		...
		IF lRet		
			IF EMPTY(POSICIONE("SZ0",1,XFILIAL("SZ0")+ALLTRIM(M->Z6_EMPCONS),"Z0_RAZAO"))
				ApMsgAlert("Empresa não cadastrada.")
				lRet := .F.
			ELSE
...
If ParamBox( aBox,cCadastro,aParam,,,,,,,,.F.,.T.)
    ...
    ELSE
		...
		IF lContinua       
		    // Localizo origem
			cQ := "SELECT Z6_CODIGO, Z6_DESCRI, Z6_EMP, Z6_SEQ, Z6_EMPCONS, Z6_TPSALDO, Z6_PER, Z6_EMPPAR, R_E_C_N_O_ AS REG  FROM "+RetSqlName("SZ6")
			cQ += " WHERE Z6_FILIAL = '"+XFILIAL("SZ6")+"' "
...
TRCell():New(oSection1,"NOME2"    ,,""              ,/*Picture*/,50,.F., {|| POSICIONE("SZ0",1,XFILIAL("SZ0")+ALLTRIM(IMPCONS->Z6_EMPCONS),"Z0_RAZAO")})
TRCell():New(oSection1,"TPSALDO"  ,,"Tipo de Saldo" ,/*Picture*/,15,.F., {|| IMPCONS->Z6_TPSALDO})
TRCell():New(oSection1,"PERC"     ,,"Percentual"    ,/*Picture*/,15,.F., {|| cPerc:= TRANSFORM(IMPCONS->Z6_PER,"@E 999.99")+"%"})
...
cQ := "SELECT Z6_EMP, Z6_SEQ , Z6_EMPCONS, Z6_TPSALDO, Z6_PER"
...
While !oReport:Cancel() .and. !IMPCONS->(Eof())
	...
	IF cEtapa <> IMPCONS->Z6_EMP
		...
		oReport:SkipLine()   
		oReport:PrintText("ETAPA: "+IMPCONS->Z6_SEQ+" - "+IMPCONS->Z6_EMP+" - "+POSICIONE("SZ0",1,XFILIAL("SZ0")+ALLTRIM(IMPCONS->Z6_EMP),"Z0_RAZAO") )
		oReport:SkipLine()   
	ENDIF
