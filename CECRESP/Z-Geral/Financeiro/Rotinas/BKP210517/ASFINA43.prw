	...
	DO WHILE !EOF()
		...
		IF IsMark("E2_OK",cMark )
			...
			lIRPFBaixa :=	Posicione("SA2",1,xfilial("SA2") + SE2->(E2_FORNECE+E2_LOJA),"A2_CALCIRF") == "2" .And. ;
								iIf( ! Empty( SED->( FieldPos( "ED_CALCIRF" ) ) ), Posicione("SED",1,FWxfilial("SED",SE2->E2_FILIAL) + SE2->(E2_NATUREZ),"ED_CALCIRF") = "S", .F.) 
              	
             	
             	
           	lCalcIssBx := IIf( ! Empty( SA2->( FieldPos( "A2_TIPO" ) ) ),Posicione("SA2",1,xFilial("SA2")+SE2->(E2_FORNECE+E2_LOJA),"A2_TIPO") == "J", .F.) .And.;
			...
			If (lContrRet .And. lPccBaixa) .Or. lIRPFBaixa .Or. lCalcIssBx .Or. lInssBx 
					...
					If (SE2->(E2_PIS+E2_COFINS+E2_CSLL) > 0) .Or. lIRPFBaixa .Or. lCalcIssBx .Or. lInssBx
						aadd(aRetencao, {SE2->(Recno()),SE2->E2_FORNECE,SE2->E2_LOJA,DTOS(SE2->E2_VENCREA),STRZERO(SE2->E2_SALDO * 100,17),0,lIRPFBaixa})
					Endif
